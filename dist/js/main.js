  ...
  _stColor = c;
    ...
    `,document.body.appendChild(this.overlay),this.zoomImg=this.overlay.querySelector(".image-zoom-img"),this.hint=this.overlay.querySelector(".image-zoom-hint"),this.bindOverlayEvents()}openOverlay(e){this.overlay||this.buildOverlay(),this.overlay.style.display="flex",this.zoomImg.src=e,this.scale=1,this.lastPos.x=0,this.lastPos.y=0,this.applyTransform(),this.hint&&(this.hint.style.opacity="1",this.hint.style.transition="opacity .5s",clearTimeout(this.hint._hideTimer),this.hint._hideTimer=setTimeout(()=>{this.hint.style.opacity="0"},3e3))}closeOverlay(){this.overlay&&(this.overlay.style.display="none",this.zoomImg.src="")}applyTransform(){this.zoomImg.style.transform=`translate(${this.lastPos.x}px, ${this.lastPos.y}px) scale(${this.scale})`}onWheel(e){e.preventDefault();const t=this.zoomImg.getBoundingClientRect(),n=e.clientX-t.left-t.width/2,i=e.clientY-t.top-t.height/2,a=e.deltaY>0?-.12:.12,c=Math.max(this.minScale,Math.min(this.maxScale,this.scale+a)),r=c/this.scale;this.lastPos.x=(this.lastPos.x+n)*r-n,this.lastPos.y=(this.lastPos.y+i)*r-i,this.scale=c,this.applyTransform(),this.hint&&(this.hint.style.opacity="0.3",clearTimeout(this.wheelTimeout),this.wheelTimeout=setTimeout(()=>{this.hint.style.opacity="1"},400))}onMouseDown(e){e.button===0&&(this.dragging=!0,this.origin.x=e.clientX,this.origin.y=e.clientY,this.overlay.style.cursor="grabbing")}onMouseMove(e){if(!this.dragging)return;const t=e.clientX-this.origin.x,n=e.clientY-this.origin.y;this.origin.x=e.clientX,this.origin.y=e.clientY,this.lastPos.x+=t,this.lastPos.y+=n,this.applyTransform()}onMouseUp(){this.dragging=!1,this.overlay&&(this.overlay.style.cursor="default")}onDblClick(){this.closeOverlay()}onKey(e){e.key==="Escape"&&this.overlay&&this.overlay.style.display==="flex"&&this.closeOverlay()}bindOverlayEvents(){this.zoomImg.addEventListener("wheel",e=>this.onWheel(e),{passive:!1}),this.zoomImg.addEventListener("mousedown",e=>this.onMouseDown(e)),this.zoomImg.addEventListener("dblclick",()=>this.onDblClick()),window.addEventListener("mousemove",e=>this.onMouseMove(e)),window.addEventListener("mouseup",()=>this.onMouseUp()),window.addEventListener("keydown",e=>this.onKey(e)),this.overlay.addEventListener("click",e=>{e.target===this.overlay&&this.closeOverlay()})}bindImages(){const e="article img, .markdown-body img, .post img, .entry-content img, .content img, .main-content img, .page img";document.querySelectorAll(e).forEach(n=>{n.classList.contains("image-zoomable")||(n.classList.add("image-zoomable"),n.style.cursor="zoom-in",n.addEventListener("click",()=>{this.openOverlay(n.getAttribute("data-origin")||n.src)}))})}}class yt{constructor(){this.initializeEmbeds()}initializeEmbeds(){this.processTextNodes(document.body),this.processCodeBlocks(),this.processMarkdownSyntax()}processTextNodes(e){["SCRIPT","STYLE","NOSCRIPT","IFRAME","CANVAS"].includes(e.tagName)||(e.nodeType===Node.TEXT_NODE?this.processTextNode(e):Array.from(e.childNodes).forEach(t=>{this.processTextNodes(t)}))}processTextNode(e){const t=e.textContent,n=/https?:\/\/(?:www\.)?shadertoy\.com\/view\/(\w+)(?:\?[^\s]*)?/g;let i;const a=[];for(;(i=n.exec(t))!==null;)a.push({fullMatch:i[0],shaderID:i[1],index:i.index});a.length>0&&this.replaceWithIframes(e,a)}processCodeBlocks(){document.querySelectorAll("code, pre").forEach(t=>{const n=t.textContent,i=/https?:\/\/(?:www\.)?shadertoy\.com\/view\/(\w+)(?:\?[^\s]*)?/g;let a;for(;(a=i.exec(n))!==null;){const c=n.trim();if(c===a[0]||c===a[0].replace(/\?.*$/,"")){this.replaceElementWithIframe(t,a[1]);break}}})}replaceWithIframes(e,t){const n=e.parentNode;if(!n)return;const i=e.textContent,a=[];let c=0;t.sort((l,o)=>o.index-l.index),t.reverse().forEach(l=>{l.index>c&&a.unshift({type:"text",content:i.substring(c,l.index)}),a.unshift({type:"iframe",shaderID:l.shaderID,originalURL:l.fullMatch}),c=l.index+l.fullMatch.length}),c<i.length&&a.unshift({type:"text",content:i.substring(c)});const r=[];a.forEach(l=>{if(l.type==="text"&&l.content.trim())r.push(document.createTextNode(l.content));else if(l.type==="iframe"){const o=this.createShaderToyEmbed(l.shaderID,l.originalURL);r.push(o)}}),r.forEach(l=>{n.insertBefore(l,e)}),n.removeChild(e)}replaceElementWithIframe(e,t){const n=this.createShaderToyEmbed(t);e.parentNode.replaceChild(n,e)}createShaderToyEmbed(e,t=null){const n=document.createElement("div");n.className="shadertoy-embed-container",n.style.cssText=`
        ...
        `;const a=document.createElement("div");a.style.cssText=`
            ...
            align-items: center;
            gap: 10px;
        `;const c=document.createElement("span");c.innerHTML="🎨",c.style.cssText=`
            font-size: 20px;
        ...
        `,a.appendChild(c),a.appendChild(r);const l=document.createElement("div");l.style.cssText=`
            display: flex;
            gap: 8px;
        `;const o=document.createElement("a");o.href=t||`https://www.shadertoy.com/view/${e}`,o.target="_blank",o.innerHTML="🔗 Open in ShaderToy",o.style.cssText=`
            ...
            align-items: center;
            gap: 4px;
        `,o.addEventListener("mouseenter",()=>{o.style.background="#66b3ff",o.style.color="#000",o.style.transform="translateY(-1px)"}),o.addEventListener("mouseleave",()=>{o.style.background="rgba(102,179,255,0.1)",o.style.color="#66b3ff",o.style.transform="translateY(0)"}),l.appendChild(o),i.appendChild(a),i.appendChild(l);const p=document.createElement("div");p.style.cssText=`
            position: relative;
            width: 100%;
            height: 0;
    ...
    `,i.querySelector(".canvas-modal__viewport").appendChild(n),document.body.appendChild(i),document.body.classList.add("canvas-modal-open");const a=new kt(n),c=l=>{l.key==="Escape"&&r()},r=()=>{a.destroy(),i.remove(),document.body.classList.remove("canvas-modal-open"),document.removeEventListener("keydown",c)};i.querySelector(".canvas-modal__close").addEventListener("click",r),i.querySelector(".canvas-modal__overlay").addEventListener("click",r),document.addEventListener("keydown",c),i.querySelectorAll(".canvas-modal__btn").forEach(l=>{l.addEventListener("click",()=>{const o=l.dataset.action;o==="zoom-in"?a.zoomBy(Se):o==="zoom-out"?a.zoomBy(1/Se):o==="reset"&&a.reset()})})}}function Ct(s){const e=s.querySelector("#canvas-arrow");if(!e)return;Ce+=1;const t=`${St}${Ce}`;e.id=t,s.querySelectorAll("[marker-end]").forEach(n=>{n.setAttribute("marker-end",`url(#${t})`)})}class kt{constructor(e){this.svg=e;const t=e.viewBox.baseVal;this.original={x:t.x,y:t.y,w:t.width,h:t.height},this.state={...this.original},this.pointers=new Map,this.pinch=null,this.svg.style.cursor="grab",this.svg.style.touchAction="none",this.onWheel=this.onWheel.bind(this),this.onPointerDown=this.onPointerDown.bind(this),this.onPointerMove=this.onPointerMove.bind(this),this.onPointerUp=this.onPointerUp.bind(this),this.svg.addEventListener("wheel",this.onWheel,{passive:!1}),this.svg.addEventListener("pointerdown",this.onPointerDown),this.svg.addEventListener("pointermove",this.onPointerMove),this.svg.addEventListener("pointerup",this.onPointerUp),this.svg.addEventListener("pointercancel",this.onPointerUp)}setViewBox(){const{x:e,y:t,w:n,h:i}=this.state;this.svg.setAttribute("viewBox",`${e} ${t} ${n} ${i}`)}currentScale(){return this.original.w/this.state.w}zoomBy(e,t,n){const i=this.currentScale()*e;i<xt||i>Et||(t==null&&(t=this.state.x+this.state.w/2),n==null&&(n=this.state.y+this.state.h/2),this.state.x=t-(t-this.state.x)/e,this.state.y=n-(n-this.state.y)/e,this.state.w/=e,this.state.h/=e,this.setViewBox())}pan(e,t){this.state.x-=e,this.state.y-=t,this.setViewBox()}reset(){this.state={...this.original},this.setViewBox()}screenToSvg(e,t){const n=this.svg.createSVGPoint();n.x=e,n.y=t;const i=this.svg.getScreenCTM();return i?n.matrixTransform(i.inverse()):{x:0,y:0}}onWheel(e){e.preventDefault();const t=e.deltaY<0?Le:1/Le,{x:n,y:i}=this.screenToSvg(e.clientX,e.clientY);this.zoomBy(t,n,i)}onPointerDown(e){e.target.closest("a")||(this.svg.setPointerCapture(e.pointerId),this.pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY,svg:this.screenToSvg(e.clientX,e.clientY)}),this.pointers.size===2?this.pinch=this.computePinch():this.pointers.size===1&&(this.svg.style.cursor="grabbing"))}onPointerMove(e){const t=this.pointers.get(e.pointerId);if(t){if(t.clientX=e.clientX,t.clientY=e.clientY,this.pointers.size===2&&this.pinch){const n=this.computePinch(),i=n.dist/this.pinch.dist;if(i>0&&Number.isFinite(i)){const a=this.screenToSvg(n.cx,n.cy);this.zoomBy(i,a.x,a.y)}this.pinch=n}else if(this.pointers.size===1){const n=this.screenToSvg(e.clientX,e.clientY);this.pan(n.x-t.svg.x,n.y-t.svg.y)}}}onPointerUp(e){this.pointers.delete(e.pointerId),this.pointers.size<2&&(this.pinch=null),this.pointers.size===0&&(this.svg.style.cursor="grab")}computePinch(){const[e,t]=[...this.pointers.values()],n=t.clientX-e.clientX,i=t.clientY-e.clientY;return{dist:Math.hypot(n,i),cx:(e.clientX+t.clientX)/2,cy:(e.clientY+t.clientY)/2}}destroy(){this.svg.removeEventListener("wheel",this.onWheel),this.svg.removeEventListener("pointerdown",this.onPointerDown),this.svg.removeEventListener("pointermove",this.onPointerMove),this.svg.removeEventListener("pointerup",this.onPointerUp),this.svg.removeEventListener("pointercancel",this.onPointerUp)}}const ue={en:{Home:"Home",Daily:"Daily",Archives:"Archives",About:"About",Portfolio:"Portfolio",Bilibili:"Bilibili",GitHub:"GitHub",Instagram:"Instagram",Douban:"Douban",Email:"Email",RSS:"RSS",Language:"Language",Copyright:"Copyright","Powered by":"Powered by","Modified based on":"Modified based on",theme:"theme","Mainly maintained using AI":"Mainly maintained using AI","Older Posts":"Older Posts","Newer Posts":"Newer Posts",Comments:"Comments","Switch to Chinese":"Switch to Chinese","Switch to English":"Switch to English","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"Switched to English",articleLanguageSwitched:"Switched to English version","tag-graph-hint":"Click tag to explore · Scroll to zoom · Drag to rearrange"},"zh-CN":{Home:"首页",Daily:"资讯",Archives:"归档",About:"关于",Portfolio:"作品集",Bilibili:"哔哩哔哩",GitHub:"GitHub",Instagram:"Instagram",Douban:"豆瓣",Email:"邮箱",RSS:"RSS",Language:"语言",Copyright:"版权所有","Powered by":"技术支持","Modified based on":"基于",theme:"主题","Mainly maintained using AI":"主要使用AI来维护","Older Posts":"上一页","Newer Posts":"下一页",Comments:"留言","Switch to Chinese":"切换至中文","Switch to English":"切换至英文","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"已切换至中文",articleLanguageSwitched:"已切换至中文版本","tag-graph-hint":"点击标签探索 · 滚轮缩放 · 拖拽移动"}},Mt=()=>(navigator.language||navigator.userLanguage).startsWith("zh")?"zh-CN":"en",Pe=()=>localStorage.getItem("siteLanguage")||Mt(),Tt=()=>{const s=document.cookie.match(/(?:^|;\s*)lang_pref=([^;]+)/);return s?s[1]:null},At=s=>{document.cookie="lang_pref="+s+"; path=/; max-age=31536000; samesite=lax"},Ie=()=>{const s=document.querySelector('meta[name="article:lang"]');return s?s.content:window.location.pathname.includes(".zh-CN")?"zh-CN":"en"},Re=s=>{const e=document.querySelector(`link[rel="alternate"][hreflang="${s}"]`);if(!e)return null;const t=new URL(e.href,window.location.origin);return window.location.origin+t.pathname+t.search+t.hash},Ne=s=>{const e=window.location.pathname;if(s==="zh-CN")return e.startsWith("/zh-CN/")||e==="/zh-CN"?null:window.location.origin+"/zh-CN"+e+window.location.search+window.location.hash;if(!e.startsWith("/zh-CN/")&&e!=="/zh-CN")return null;const t=e.replace(/^\/zh-CN(?=\/|$)/,"")||"/";return window.location.origin+t+window.location.search+window.location.hash},ze=s=>{document.querySelectorAll(".lang-switch__opt").forEach(e=>{e.setAttribute("aria-pressed",e.getAttribute("data-lang")===s?"true":"false")})},fe=s=>{const e=ue[s];if(!e){console.warn("Language data not available for:",s);return}document.documentElement.lang=s,document.querySelectorAll("nav ul li a").forEach(l=>{const o=l.getAttribute("data-i18n-key");o&&e[o]&&(l.textContent=e[o])}),document.querySelectorAll("[data-i18n]").forEach(l=>{const o=l.getAttribute("data-i18n");e[o]&&(l.textContent=e[o])}),document.querySelectorAll("[data-title]").forEach(l=>{const o=l.getAttribute("data-title");e[o]&&l.setAttribute("data-title",e[o])});const a=document.querySelector(".pagination .extend.prev"),c=document.querySelector(".pagination .extend.next");a&&(a.textContent=e["Older Posts"]||a.textContent),c&&(c.textContent=e["Newer Posts"]||c.textContent),localStorage.setItem("siteLanguage",s),document.querySelectorAll("[data-i18n-tag]").forEach(l=>{const o=l.getAttribute("data-i18n-tag");if(s==="zh-CN"){const p=window.tagTranslations&&window.tagTranslations[o];p&&(l.textContent=p)}else l.textContent=o}),ze(s)},Pt=s=>{const e=document.querySelector(".lang-notification");e&&e.remove();const t=document.createElement("div");t.className="lang-notification",t.textContent=s,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.parentNode&&t.parentNode.removeChild(t)},300)},2e3)},_e=s=>{if(At(s),s===Ie()){localStorage.setItem("siteLanguage",s),fe(s);return}const e=Re(s)||Ne(s);if(e){localStorage.setItem("siteLanguage",s),window.location.href=e;return}fe(s);const t=ue[s]?ue[s].languageSwitched:"Language switched";Pt(t)},It=()=>{const s=Pe()==="zh-CN"?"en":"zh-CN";_e(s)},Rt=()=>{document.querySelectorAll(".lang-switch__opt").forEach(s=>{s.addEventListener("click",e=>{e.preventDefault(),_e(s.getAttribute("data-lang"))})}),ze(Pe())},ke=()=>{const s=Ie(),e=Tt();if(fe(e||s),e&&e!==s){const t=Re(e)||Ne(e);t&&window.location.replace(t)}};function Nt(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ke):ke(),window.addEventListener("load",()=>{const s=document.getElementById("langSwitch");s&&s.addEventListener("click",e=>{e.preventDefault(),It()}),Rt()})}function zt(){const s=document.getElementById("tag-graph"),e=document.getElementById("tag-graph-container"),t=window.__TAG_GRAPH_DATA__;if(!s||!t||!t.nodes||t.nodes.length===0)return;const n=document.createElement("div");n.className="graph-loading",n.textContent="Loading",s.parentNode.appendChild(n);const i=t.archiveFilterTags||[],a={};i.forEach(function(d){a[d]=!0});function c(d){return String(d||"").replace(/-/g," ").replace(/\s+/g," ").trim().toLowerCase()}const r={};t.links.forEach(function(d){r[d.source]=(r[d.source]||0)+1,r[d.target]=(r[d.target]||0)+1});let l=1;t.nodes.forEach(function(d){const h=d.value||0;h>l&&(l=h)}),t.nodes.forEach(function(d){const h=d.value||0;d.symbolSize=Math.max(12,Math.min(70,12+h*(58/l)))});const o={};t.nodes.forEach(function(d){o[d.name]=[]});function p(d,h){!o[d]||!o[h]||d===h||(o[d].indexOf(h)===-1&&o[d].push(h),o[h].indexOf(d)===-1&&o[h].push(d))}t.links.forEach(function(d){p(d.source,d.target)});const k={};t.nodes.forEach(function(d){const h=c(d.name);k[h]||(k[h]=[]),k[h].push(d.name)}),Object.keys(k).forEach(function(d){const h=k[d];if(!(h.length<2))for(let v=0;v<h.length;v++)for(let b=v+1;b<h.length;b++)p(h[v],h[b])});const u={},S=[];i.forEach(function(d){o[d]!==void 0&&(u[d]=0,S.push(d))});let T=0;for(;T<S.length;){const d=S[T++];(o[d]||[]).forEach(function(h){u[h]===void 0&&(u[h]=u[d]+1,S.push(h))})}Object.keys(u).forEach(function(d){});const N=[{h:260,s:62,l:50},{h:15,s:80,l:55},{h:160,s:60,l:42},{h:220,s:72,l:52},{h:340,s:70,l:52},{h:45,s:85,l:50},{h:190,s:70,l:45},{h:90,s:55,l:45},{h:290,s:60,l:50},{h:30,s:75,l:48},{h:130,s:50,l:42},{h:0,s:70,l:55}];function Y(d,h,v){return"hsl("+Math.round(d)+", "+Math.round(h)+"%, "+Math.round(v)+"%)"}const $={},oe={},D=t.nodes.filter(function(d){return a[d.name]});D.sort(function(d,h){return h.value-d.value}),D.forEach(function(d,h){const v=N[h%N.length];oe[d.name]=v,$[d.name]=Y(v.h,v.s,v.l)});const F={};i.forEach(function(d){if(o[d]===void 0)return;const h={};h[d]=0;const v=[d];let b=0;for(;b<v.length;){const P=v[b++];(o[P]||[]).forEach(function(L){h[L]===void 0&&(h[L]=h[P]+1,v.push(L))})}F[d]=h});const Q=t.nodes.filter(function(d){return!a[d.name]});let re=1;Q.forEach(function(d){let h=1/0;i.forEach(function(v){if(!F[v])return;const b=F[v][d.name];b!==void 0&&b<h&&(h=b)}),h<1/0&&h>re&&(re=h)}),Q.forEach(function(d){const h=[];let v=0;if(i.forEach(function(f){if(!F[f]||!oe[f])return;let g=F[f][d.name];if(g===void 0)return;g===0&&(g=.5);const x=1/(g*g);h.push({ft:f,w:x}),v+=x}),v===0){$[d.name]="hsl(0, 0%, 82%)";return}let b=0,P=0,L=0,y=0;h.forEach(function(f){const g=f.w/v,x=oe[f.ft],R=x.h*Math.PI/180;b+=Math.sin(R)*g,P+=Math.cos(R)*g,L+=x.s*g,y+=x.l*g});let C=Math.atan2(b,P)*180/Math.PI;C<0&&(C+=360);let z=L,_=y,m=1/0;h.forEach(function(f){const g=F[f.ft][d.name];g<m&&(m=g)});let w=(m-1)/Math.max(re-1,1);w=Math.max(0,Math.min(1,w));const A=Math.pow(w,.85),M=32,E=1-A*.35;z=Math.max(M,z*E),_=_+A*(82-_)*.78,$[d.name]=Y(C,z,_)});const le=t.tagTranslations||{},J={};Object.keys(le).forEach(function(d){J[d]=le[d];const h=d.replace(/-/g," ");h!==d&&(J[h]=le[d])});function ee(d){return(typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en")==="zh-CN"&&J[d]?J[d]:d}const me=e.getBoundingClientRect(),H=me.width||500,O=me.height||400,ve=Math.min(H,O)*.28;D.forEach(function(d,h){const v=2*Math.PI*h/Math.max(D.length,1)-Math.PI/2;d.x=H/2+ve*Math.cos(v),d.y=O/2+ve*Math.sin(v)});const ge=Math.min(H,O)*.45;Q.forEach(function(d,h){const v=2*Math.PI*h/Math.max(Q.length,1);d.x=H/2+ge*Math.cos(v),d.y=O/2+ge*Math.sin(v)});const Oe=60,I=t.nodes;for(let d=0;d<15;d++)for(let h=0;h<I.length;h++)for(let v=h+1;v<I.length;v++){const b=I[v].x-I[h].x,P=I[v].y-I[h].y,L=Math.sqrt(b*b+P*P),y=Oe+(I[h].symbolSize+I[v].symbolSize)/2;if(L<y){const C=(y-L)/2,z=L>.1?b/L:Math.random()-.5,_=L>.1?P/L:Math.random()-.5;I[h].x-=z*C,I[h].y-=_*C,I[v].x+=z*C,I[v].y+=_*C}}let X=1,K=[H/2,O/2];if(D.length>0){let d=1/0,h=-1/0,v=1/0,b=-1/0;D.forEach(function(y){const C=(y.symbolSize||20)/2+50;y.x-C<d&&(d=y.x-C),y.x+C>h&&(h=y.x+C),y.y-C<v&&(v=y.y-C),y.y+C>b&&(b=y.y+C)});const P=h-d,L=b-v;if(P>0&&L>0){const y=H/P,C=O/L;X=Math.min(y,C,1.5)*.8,X<.3&&(X=.3),K=[(d+h)/2,(v+b)/2]}}t.nodes.forEach(function(d){d.itemStyle={color:$[d.name],borderColor:"#fff",borderWidth:1.5,shadowBlur:5,shadowColor:"rgba(0, 0, 0, 0.06)"},d.label={show:!0,formatter:function(){return ee(d.name)},fontSize:Math.max(10,Math.min(15,9+(r[d.name]||0)*.5)),color:"#555",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'}});const B=e.querySelector(".tag-graph-hint");let ye=null,ce=!1;function Fe(){ce||(ce=!0,B.classList.add("visible"),clearTimeout(ye),ye=setTimeout(function(){B.classList.remove("visible"),ce=!1},3e3))}e.addEventListener("mouseenter",Fe);const W=document.createElement("script");W.src="https://cdn.jsdelivr.net/npm/echarts@5.5.1/dist/echarts.min.js",W.integrity="sha384-Mx5lkUEQPM1pOJCwFtUICyX45KNojXbkWdYhkKUKsbv391mavbfoAmONbzkgYPzR",W.crossOrigin="anonymous",W.onload=function(){We()},W.onerror=function(){n.textContent="Failed to load chart library",n.style.color="#c44"},document.head.appendChild(W);function Be(d){return d<10?750:d<20?1200:d<40?1650:2100}function We(){n.parentNode&&n.parentNode.removeChild(n);const d=window.matchMedia&&window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window,h=echarts.init(s),v={backgroundColor:"transparent",tooltip:{show:!0,enterable:!0,confine:!0,backgroundColor:"rgba(255, 255, 255, 0.97)",borderColor:"#e8e8e8",borderWidth:1,padding:[10,14],textStyle:{color:"#4b4848",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',fontSize:13},extraCssText:"border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); max-height: 260px; overflow-y: auto;",formatter:function(m){function w(M){return String(M??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function A(M,E){const f=w(M),g=w(E),x='style="display:block;color:#666;font-size:11px;line-height:1.6;padding:1px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px;text-decoration:none;"';return g?'<a href="'+g+'" '+x+">• "+f+"</a>":"<div "+x+">• "+f+"</div>"}if(m.dataType==="node"){const M=ee(m.name);let E='<div style="font-weight:600;font-size:14px;margin-bottom:5px;color:'+($[m.name]||"#795da3")+'">'+w(M)+"</div>";E+='<div style="color:#777;font-size:12px;margin-bottom:6px;">📄 '+m.value+" article"+(m.value>1?"s":"")+"</div>";const f=t.postTitles&&t.postTitles[m.name];return f&&f.length>0&&(E+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;">',f.forEach(function(g){typeof g=="string"?E+=A(g,""):E+=A(g.title,g.path)}),E+="</div>"),E}if(m.dataType==="edge"){const M=m.data.source,E=m.data.target;let f='<span style="font-weight:600">'+w(ee(M))+'</span> <span style="color:#bbb">↔</span> <span style="font-weight:600">'+w(ee(E))+"</span>";f+='<br/><span style="color:#999;font-size:12px">📄 '+m.data.value+" article"+(m.data.value>1?"s":"")+"</span>";const g=[M,E].sort().join("	"),x=t.linkPosts&&t.linkPosts[g];return x&&x.length>0&&(f+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;margin-top:5px;">',x.forEach(function(R){f+=A(R.title,R.path)}),f+="</div>"),f}}},animationDuration:1500,animationEasingUpdate:"quinticInOut",series:[{type:"graph",layout:"force",data:t.nodes,links:t.links,roam:!1,draggable:!d,force:{repulsion:Be(t.nodes.length),edgeLength:[150,450],gravity:.12,friction:.6,layoutAnimation:!0},emphasis:{focus:"adjacency",blurScope:"global",itemStyle:{shadowBlur:20,shadowColor:"rgba(121, 93, 163, 0.45)",borderWidth:2,borderColor:"#fff"},lineStyle:{width:3,opacity:.85},label:{show:!0,fontSize:14,fontWeight:"bold",color:"#333"}},label:{position:"right",distance:6},lineStyle:{color:"#d0d0d0",width:1.5,curveness:0,opacity:.35},scaleLimit:{min:.3,max:4},zoom:X,center:K}]};h.setOption(v);let b=!1,P=!1;if(D.length>0){const m=function(){if(P)return;if(b){P=!0,h.off("finished",m);return}const w=h.getModel(),A=w&&w.getSeriesByIndex&&w.getSeriesByIndex(0),M=A&&A.getGraph&&A.getGraph();let E=1/0,f=-1/0,g=1/0,x=-1/0,R=0;if(D.forEach(function(ne){const de=M&&M.getNodeByName&&M.getNodeByName(ne.name),ie=de&&de.getLayout&&de.getLayout();let j,G;ie&&ie.length>=2?(j=ie[0],G=ie[1]):(j=ne.x||0,G=ne.y||0);const q=(ne.symbolSize||20)/2+50;j-q<E&&(E=j-q),j+q>f&&(f=j+q),G-q<g&&(g=G-q),G+q>x&&(x=G+q),R++}),R===0)return;const Z=f-E,te=x-g;if(Z<=0||te<=0)return;let U=Math.min(H/Z,O/te,1.5)*.8;U<.3&&(U=.3),P=!0,L=U,y=[(E+f)/2,(g+x)/2],h.setOption({series:[{zoom:U,center:y.slice()}]}),h.off("finished",m)};h.on("finished",m)}h.on("click",function(m){m.dataType==="node"&&t.tagPaths&&t.tagPaths[m.name]&&(window.location.href=t.tagPaths[m.name])}),h.on("mouseover",function(m){(m.dataType==="node"||m.dataType==="edge")&&(s.style.cursor="pointer")}),h.on("mouseout",function(){s.style.cursor="default"});let L=X||1,y=K?[K[0],K[1]]:[0,0];if(!d){const m=e||s;m.addEventListener("wheel",function(f){f.preventDefault()},{passive:!1}),m.addEventListener("touchmove",function(f){f.touches.length>=2&&f.preventDefault()},{passive:!1});const w=h.getZr();w.on("mousewheel",function(f){f.event.preventDefault(),f.event.stopPropagation(),b=!0;const g=f.wheelDelta>0?1.08:1/1.08;let x=L*g;x<.3&&(x=.3),x>4&&(x=4),L=x,h.setOption({series:[{zoom:L}]})});let A=!1,M=[0,0],E=[0,0];w.on("mousedown",function(f){f.target||(A=!0,b=!0,M=[f.event.clientX,f.event.clientY],E=[y[0],y[1]],s.style.cursor="grabbing")}),w.on("mousemove",function(f){if(A){const g=f.event.clientX-M[0],x=f.event.clientY-M[1],R=s.clientWidth,Z=s.clientHeight,te=R/L,U=Z/L;y[0]=E[0]-g*(te/R),y[1]=E[1]-x*(U/Z),h.setOption({series:[{center:[y[0],y[1]]}]})}}),w.on("mouseup",function(){A&&(A=!1,s.style.cursor="default")}),w.on("globalout",function(){A&&(A=!1,s.style.cursor="default")})}if(d){const m=document.createElement("button");m.type="button",m.className="tag-graph-fs-btn",m.setAttribute("aria-label","Fullscreen"),m.innerHTML='<svg class="tag-graph-fs-ic tag-graph-fs-ic--open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg><svg class="tag-graph-fs-ic tag-graph-fs-ic--close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',e.appendChild(m);let w=!1;const A=function(){if(!B)return;const f=typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en";B.textContent=f==="zh-CN"?"双指缩放 · 拖动平移 · 点按进入标签":"Pinch to zoom · Drag to pan · Tap a tag",B.classList.add("visible"),setTimeout(function(){B.classList.remove("visible")},2600)},M=function(){w=!0,e.classList.add("tag-graph-fullscreen"),m.classList.add("is-fullscreen"),m.setAttribute("aria-label","Exit fullscreen"),document.body.style.overflow="hidden",document.body.classList.add("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!0}]}),A()})},E=function(){w=!1,e.classList.remove("tag-graph-fullscreen"),m.classList.remove("is-fullscreen"),m.setAttribute("aria-label","Fullscreen"),document.body.style.overflow="",document.body.classList.remove("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!1,zoom:L,center:y.slice()}]})})};m.addEventListener("click",function(f){f.preventDefault(),f.stopPropagation(),w?E():M()}),document.addEventListener("keydown",function(f){f.key==="Escape"&&w&&E()})}let C;window.addEventListener("resize",function(){clearTimeout(C),C=setTimeout(function(){h.resize()},150)});function z(){h.setOption({series:[{data:t.nodes}]})}window.addEventListener("storage",function(m){m.key==="siteLanguage"&&z()});const _=localStorage.setItem;localStorage.setItem=function(m,w){_.call(localStorage,m,w),m==="siteLanguage"&&setTimeout(z,50)}}}function _t(s){let e=s.trim();const t=e.indexOf(" — ");t>0&&(e=e.slice(0,t));const n=e.indexOf("（");return n>6&&(e=e.slice(0,n)),e.trim()}function Dt(s){if(s.querySelector("audio"))return!1;const e=s.querySelector("summary");return!(!e||/跟读|本节语音/.test(e.textContent||"")||s.parentElement&&s.parentElement.closest("details.callout"))}function qt(s,e={}){const t=e.includeCallouts?"h1, h2, h3, h4, h5, h6, details.callout--foldable":"h1, h2, h3, h4, h5, h6",n=Array.from(s.querySelectorAll(t)),i=[];let a=1;const c=[];return n.forEach(r=>{const l=/^H[1-6]$/.test(r.tagName);let o,p,k=!1;if(l)o=parseInt(r.tagName[1],10),a=o,p=r.textContent;else{if(!Dt(r))return;o=a+1,k=!0;const T=r.querySelector("summary");if(p=_t(T.textContent||""),!p)return}let u=1;for(;i.length&&i[i.length-1].level>=o;){const T=i.pop();T.level===o&&(u=T.n+1)}i.push({level:o,n:u});const S=c.length;r.id||(r.id=k?`toc-item-${S}`:`heading-${S}`),c.push({element:r,level:o,index:S,id:r.id,text:p,number:i.map(T=>T.n).join("."),virtual:k})}),c}function Ht(s){const e=document.createElement("aside");e.className="toc-drawer",e.innerHTML='<div class="toc-header"><span class="toc-title"></span><button type="button" class="toc-close-btn"><span class="toc-close-btn__x" aria-hidden="true"></span></button></div><div class="toc-content"><div class="toc-list"></div></div>',document.body.appendChild(e);const t=e.querySelector(".toc-list"),n=s.map(i=>{const a=document.createElement("div");if(a.className=i.virtual?"toc-item toc-item--virtual":"toc-item",a.setAttribute("data-level",String(i.level)),a.setAttribute("data-index",String(i.index)),!i.virtual){const r=document.createElement("div");r.className="toc-collapse-btn",a.appendChild(r)}const c=document.createElement("span");return c.className="toc-item-text",c.style.cursor="pointer",c.innerHTML=`<span class="toc-number">${i.number}.</span> `,c.appendChild(document.createTextNode(i.text)),c.setAttribute("title",i.text),i.element.classList.contains("collapsed")&&a.classList.add("collapsed"),a.appendChild(c),t.appendChild(a),a});return{container:e,items:n}}const De="toc-panel-state";function qe(){try{const s=localStorage.getItem(De);if(!s)return null;const e=JSON.parse(s);return!e||typeof e!="object"?null:e}catch{return null}}function Ot(s){try{const t={...qe()||{},...s};localStorage.setItem(De,JSON.stringify(t))}catch{}}function Ft(){return(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?"zh":"en"}const Bt={zh:{title:"目录",open:"目录",hide:"收起目录",show:"打开目录"},en:{title:"Contents",open:"TOC",hide:"Hide contents",show:"Show contents"}};function Wt(s){const e=s.querySelector(".toc-content"),t=s.querySelector(".toc-item.toc-reading");!e||!t||(e.scrollTop=t.offsetTop-e.clientHeight/2+t.offsetHeight/2)}function Ut(s){const e=Bt[Ft()],t=s.querySelector(".toc-title");t&&(t.textContent=e.title),s.setAttribute("aria-label",e.title);const n=s.querySelector(".toc-close-btn");n&&(n.setAttribute("aria-label",e.hide),n.setAttribute("title",e.hide));const i=document.createElement("button");i.type="button",i.className="toc-tab",i.setAttribute("aria-label",e.show),i.innerHTML='<span class="toc-tab__icon" aria-hidden="true"></span><span class="toc-tab__text">'+e.open+"</span>",document.body.appendChild(i);const a=document.createElement("div");a.className="toc-scrim",document.body.appendChild(a);function c(o,p){s.classList.toggle("is-open",o),i.classList.toggle("is-hidden",o),a.classList.toggle("is-visible",o),o&&Wt(s),p&&Ot({hidden:!o})}i.addEventListener("click",()=>c(!0,!0)),a.addEventListener("click",()=>c(!1,!0)),n&&n.addEventListener("click",o=>{o.stopPropagation(),c(!1,!0)}),document.addEventListener("keydown",o=>{o.key==="Escape"&&s.classList.contains("is-open")&&c(!1,!1)});const r=qe(),l=window.matchMedia("(min-width: 1100px)").matches;return c(l&&!!r&&r.hidden===!1,!1),{setOpen:c}}function pe(s,e,t){for(let n=e+1;n<s.length&&!(parseInt(s[n].getAttribute("data-level")||"1",10)<=t);n+=1)s[n].classList.add("toc-hidden")}function He(s,e,t){for(let n=e+1;n<s.length;n+=1){const i=parseInt(s[n].getAttribute("data-level")||"1",10);if(i<=t)break;if(i===t+1)s[n].classList.remove("toc-hidden");else{let a=!0;for(let c=n-1;c>e;c-=1){const r=parseInt(s[c].getAttribute("data-level")||"1",10);if(r<i&&s[c].classList.contains("collapsed")){a=!1;break}if(r<=t)break}a&&s[n].classList.remove("toc-hidden")}}}function jt(s){const e=parseInt(s.tagName.charAt(1),10);let t=s.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t=t.nextElementSibling;continue}t.style.display="none",t=t.nextElementSibling}}function Gt(s){const e=parseInt(s.tagName.charAt(1),10);let t=s.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t.style.display="",t=t.nextElementSibling;continue}t.style.display="",t=t.nextElementSibling}}function Vt(s,e,t){const n=e[t],i=s[t]&&s[t].element;if(!n||!i)return;const a=parseInt(n.getAttribute("data-level")||"1",10);!n.classList.contains("collapsed")?(n.classList.add("collapsed"),pe(e,t,a),i.classList.contains("collapsed")||(i.classList.add("collapsed"),jt(i))):(n.classList.remove("collapsed"),He(e,t,a),i.classList.contains("collapsed")&&(i.classList.remove("collapsed"),Gt(i)))}function Yt(s,e,t){const n=e[t],i=s[t]&&s[t].element;if(!n||!i)return;const a=parseInt(n.getAttribute("data-level")||"1",10),c=i.classList.contains("collapsed");c&&!n.classList.contains("collapsed")?(n.classList.add("collapsed"),pe(e,t,a)):!c&&n.classList.contains("collapsed")&&(n.classList.remove("collapsed"),He(e,t,a))}function $t(s,e){e.forEach((i,a)=>{if(i.classList.contains("collapsed")){const c=parseInt(i.getAttribute("data-level")||"1",10);pe(e,a,c)}}),e.forEach((i,a)=>{const c=i.querySelector(".toc-collapse-btn");c&&c.addEventListener("click",r=>{r.stopPropagation(),Vt(s,e,a)})});const t=new Map;s.forEach((i,a)=>t.set(i.element,a));const n=new MutationObserver(i=>{i.forEach(a=>{if(a.type!=="attributes"||a.attributeName!=="class")return;const c=t.get(a.target);c!==void 0&&Yt(s,e,c)})});return s.forEach(i=>{n.observe(i.element,{attributes:!0,attributeFilter:["class"]})}),{observer:n}}const Me={1:{passed:"rgba(128,128,128,0.1)",reading:"rgba(66,153,225,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(66,153,225,0.25)"},2:{passed:"rgba(128,128,128,0.1)",reading:"rgba(49,130,206,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(49,130,206,0.25)"},3:{passed:"rgba(128,128,128,0.1)",reading:"rgba(44,82,130,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(44,82,130,0.25)"},4:{passed:"rgba(128,128,128,0.1)",reading:"rgba(42,67,101,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(42,67,101,0.25)"},5:{passed:"rgba(128,128,128,0.1)",reading:"rgba(26,54,93,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(26,54,93,0.25)"},6:{passed:"rgba(128,128,128,0.1)",reading:"rgba(21,62,117,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(21,62,117,0.25)"}};function Xt(s,e){if(!s.length)return{destroy(){}};const t=new Array(s.length).fill("coming"),n=new Set;let i=-1;const a=new Map;s.forEach((u,S)=>a.set(u.element,S));function c(){e.forEach((u,S)=>{const T=parseInt(u.getAttribute("data-level")||"1",10),N=Me[T]||Me[1],Y=t[S];u.classList.remove("toc-passed","toc-reading","toc-coming"),u.style.boxShadow="",u.style.transform="",u.style.fontWeight="",S===i?(u.classList.add("toc-reading"),u.style.backgroundColor=N.active,u.style.opacity="1",u.style.fontWeight="600",u.style.boxShadow="inset 0 0 0 2px rgba(66,153,225,0.3)",u.style.transform="scale(1.02)",u.style.transition="all 0.2s ease"):Y==="reading"?(u.classList.add("toc-reading"),u.style.backgroundColor=N.reading,u.style.opacity="1",u.style.fontWeight="600"):Y==="passed"?(u.classList.add("toc-passed"),u.style.backgroundColor=N.passed,u.style.opacity="0.7"):(u.classList.add("toc-coming"),u.style.backgroundColor=N.coming,u.style.opacity="0.5")})}function r(){const u=window.innerHeight/2;let S=-1;n.forEach(T=>{const N=s[T].element.getBoundingClientRect();N.top<=u&&N.bottom>=u&&(S=T)}),S!==i&&(i=S,c())}let l=null;function o(){l||(l=requestAnimationFrame(()=>{l=null,r()}))}const p=new IntersectionObserver(u=>{u.forEach(S=>{const T=a.get(S.target);T!==void 0&&(S.isIntersecting?(n.add(T),t[T]="reading"):(n.delete(T),t[T]=S.boundingClientRect.bottom<0?"passed":"coming"))}),r(),c()});s.forEach(u=>p.observe(u.element)),window.addEventListener("scroll",o,{passive:!0}),window.addEventListener("resize",o,{passive:!0}),c();function k(){p.disconnect(),window.removeEventListener("scroll",o),window.removeEventListener("resize",o),l&&cancelAnimationFrame(l)}return{destroy:k,refresh:()=>{r(),c()}}}function Kt(){const s=document.querySelector(".content");if(!s||s.classList.contains("archives"))return null;const e=document.body;if(e.classList.contains("path-about-index-html")||e.classList.contains("layout-about"))return null;const t=document.querySelector("section.main[data-toc]");return t&&t.getAttribute("data-toc")==="false"?null:s}function Zt(){const s=Kt();if(!s)return;const e=qt(s,{includeCallouts:document.body.classList.contains("type-daily-feed")});if(!e.length)return;const{container:t,items:n}=Ht(e);$t(e,n);const i=Xt(e,n);Ut(t),n.forEach((a,c)=>{const r=a.querySelector(".toc-item-text");r&&r.addEventListener("click",()=>{const l=e[c];!l||!l.element||(l.virtual&&l.element.tagName==="DETAILS"&&!l.element.open&&(l.element.open=!0),l.element.scrollIntoView({behavior:"smooth",block:l.virtual?"start":"center"}),setTimeout(()=>i.refresh(),300))})})}function Te(){new Ue,new it,new pt,new mt,document.getElementById("map")&&new vt,new gt,setTimeout(()=>{new yt},500),new bt,new wt,new Lt,Nt(),Zt(),document.getElementById("tag-graph")&&zt()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Te):Te();
