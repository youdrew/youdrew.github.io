...
`,Ve="(min-width: 1099px)",Ye="(prefers-reduced-motion: reduce)",Xe=8e3,$e=30,Ke=3.4,Ze=1.3,be=.55,xe=1100,Ee=1,Qe=3,Je=1.7;function et(){return/(^|\s)path-(zh-CN-)?index-html(\s|$)/.test(document.body.className||"")}function Z(o){return Math.max(0,Math.min(1,o))}function re(o){return o*o*(3-2*o)}const tt=`#version 300 es
in vec2 p;
...
void main() {
  ...
  _stColor = c;
    ...
    `,document.body.appendChild(this.overlay),this.zoomImg=this.overlay.querySelector(".image-zoom-img"),this.hint=this.overlay.querySelector(".image-zoom-hint"),this.bindOverlayEvents()}openOverlay(e){this.overlay||this.buildOverlay(),this.overlay.style.display="flex",this.zoomImg.src=e,this.scale=1,this.lastPos.x=0,this.lastPos.y=0,this.applyTransform(),this.hint&&(this.hint.style.opacity="1",this.hint.style.transition="opacity .5s",clearTimeout(this.hint._hideTimer),this.hint._hideTimer=setTimeout(()=>{this.hint.style.opacity="0"},3e3))}closeOverlay(){this.overlay&&(this.overlay.style.display="none",this.zoomImg.src="")}applyTransform(){this.zoomImg.style.transform=`translate(${this.lastPos.x}px, ${this.lastPos.y}px) scale(${this.scale})`}onWheel(e){e.preventDefault();const t=this.zoomImg.getBoundingClientRect(),n=e.clientX-t.left-t.width/2,i=e.clientY-t.top-t.height/2,s=e.deltaY>0?-.12:.12,r=Math.max(this.minScale,Math.min(this.maxScale,this.scale+s)),d=r/this.scale;this.lastPos.x=(this.lastPos.x+n)*d-n,this.lastPos.y=(this.lastPos.y+i)*d-i,this.scale=r,this.applyTransform(),this.hint&&(this.hint.style.opacity="0.3",clearTimeout(this.wheelTimeout),this.wheelTimeout=setTimeout(()=>{this.hint.style.opacity="1"},400))}onMouseDown(e){e.button===0&&(this.dragging=!0,this.origin.x=e.clientX,this.origin.y=e.clientY,this.overlay.style.cursor="grabbing")}onMouseMove(e){if(!this.dragging)return;const t=e.clientX-this.origin.x,n=e.clientY-this.origin.y;this.origin.x=e.clientX,this.origin.y=e.clientY,this.lastPos.x+=t,this.lastPos.y+=n,this.applyTransform()}onMouseUp(){this.dragging=!1,this.overlay&&(this.overlay.style.cursor="default")}onDblClick(){this.closeOverlay()}onKey(e){e.key==="Escape"&&this.overlay&&this.overlay.style.display==="flex"&&this.closeOverlay()}bindOverlayEvents(){this.zoomImg.addEventListener("wheel",e=>this.onWheel(e),{passive:!1}),this.zoomImg.addEventListener("mousedown",e=>this.onMouseDown(e)),this.zoomImg.addEventListener("dblclick",()=>this.onDblClick()),window.addEventListener("mousemove",e=>this.onMouseMove(e)),window.addEventListener("mouseup",()=>this.onMouseUp()),window.addEventListener("keydown",e=>this.onKey(e)),this.overlay.addEventListener("click",e=>{e.target===this.overlay&&this.closeOverlay()})}bindImages(){const e="article img, .markdown-body img, .post img, .entry-content img, .content img, .main-content img, .page img";document.querySelectorAll(e).forEach(n=>{n.classList.contains("image-zoomable")||(n.classList.add("image-zoomable"),n.style.cursor="zoom-in",n.addEventListener("click",()=>{this.openOverlay(n.getAttribute("data-origin")||n.src)}))})}}class lt{constructor(){this.initializeEmbeds()}initializeEmbeds(){this.processTextNodes(document.body),this.processCodeBlocks(),this.processMarkdownSyntax()}processTextNodes(e){["SCRIPT","STYLE","NOSCRIPT","IFRAME","CANVAS"].includes(e.tagName)||(e.nodeType===Node.TEXT_NODE?this.processTextNode(e):Array.from(e.childNodes).forEach(t=>{this.processTextNodes(t)}))}processTextNode(e){const t=e.textContent,n=/https?:\/\/(?:www\.)?shadertoy\.com\/view\/(\w+)(?:\?[^\s]*)?/g;let i;const s=[];for(;(i=n.exec(t))!==null;)s.push({fullMatch:i[0],shaderID:i[1],index:i.index});s.length>0&&this.replaceWithIframes(e,s)}processCodeBlocks(){document.querySelectorAll("code, pre").forEach(t=>{const n=t.textContent,i=/https?:\/\/(?:www\.)?shadertoy\.com\/view\/(\w+)(?:\?[^\s]*)?/g;let s;for(;(s=i.exec(n))!==null;){const r=n.trim();if(r===s[0]||r===s[0].replace(/\?.*$/,"")){this.replaceElementWithIframe(t,s[1]);break}}})}replaceWithIframes(e,t){const n=e.parentNode;if(!n)return;const i=e.textContent,s=[];let r=0;t.sort((l,a)=>a.index-l.index),t.reverse().forEach(l=>{l.index>r&&s.unshift({type:"text",content:i.substring(r,l.index)}),s.unshift({type:"iframe",shaderID:l.shaderID,originalURL:l.fullMatch}),r=l.index+l.fullMatch.length}),r<i.length&&s.unshift({type:"text",content:i.substring(r)});const d=[];s.forEach(l=>{if(l.type==="text"&&l.content.trim())d.push(document.createTextNode(l.content));else if(l.type==="iframe"){const a=this.createShaderToyEmbed(l.shaderID,l.originalURL);d.push(a)}}),d.forEach(l=>{n.insertBefore(l,e)}),n.removeChild(e)}replaceElementWithIframe(e,t){const n=this.createShaderToyEmbed(t);e.parentNode.replaceChild(n,e)}createShaderToyEmbed(e,t=null){const n=document.createElement("div");n.className="shadertoy-embed-container",n.style.cssText=`
        ...
        `,s.appendChild(r),s.appendChild(d);const l=document.createElement("div");l.style.cssText=`
            display: flex;
            gap: 8px;
        `;const a=document.createElement("a");a.href=t||`https://www.shadertoy.com/view/${e}`,a.target="_blank",a.innerHTML="🔗 Open in ShaderToy",a.style.cssText=`
            ...
            align-items: center;
            gap: 4px;
        `,a.addEventListener("mouseenter",()=>{a.style.background="#66b3ff",a.style.color="#000",a.style.transform="translateY(-1px)"}),a.addEventListener("mouseleave",()=>{a.style.background="rgba(102,179,255,0.1)",a.style.color="#66b3ff",a.style.transform="translateY(0)"}),l.appendChild(a),i.appendChild(s),i.appendChild(l);const u=document.createElement("div");u.style.cssText=`
            position: relative;
            width: 100%;
            height: 0;
            ...
            overflow: hidden;
            background: #000;
        `;const x=document.createElement("iframe");x.src=`https://www.shadertoy.com/embed/${e}?gui=true&t=10&paused=false&muted=false`,x.style.cssText=`
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            border: none;
        `,x.allowFullscreen=!0,x.loading="lazy";const p=document.createElement("div");p.innerHTML=`
        ...
        `,document.head.appendChild(k),x.addEventListener("load",()=>{p.style.display="none"}),u.appendChild(x),u.appendChild(p),n.appendChild(i),n.appendChild(u),n}processMarkdownSyntax(){const e=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode:function(i){var r;const s=(r=i.parentElement)==null?void 0:r.tagName;return["SCRIPT","STYLE","NOSCRIPT","IFRAME","CANVAS"].includes(s)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),t=[];let n;for(;n=e.nextNode();)t.push(n);t.forEach(i=>{const s=i.textContent,r=/\[(shader|shadertoy):(\w+)\]/g;let d;const l=[];for(;(d=r.exec(s))!==null;)l.push({fullMatch:d[0],shaderID:d[2],index:d.index});l.length>0&&this.replaceMarkdownSyntax(i,l)})}replaceMarkdownSyntax(e,t){const n=e.parentNode;if(!n)return;const i=e.textContent,s=[];let r=0;t.sort((l,a)=>a.index-l.index),t.reverse().forEach(l=>{l.index>r&&s.unshift({type:"text",content:i.substring(r,l.index)}),s.unshift({type:"iframe",shaderID:l.shaderID,originalURL:null}),r=l.index+l.fullMatch.length}),r<i.length&&s.unshift({type:"text",content:i.substring(r)});const d=[];s.forEach(l=>{if(l.type==="text"&&l.content.trim())d.push(document.createTextNode(l.content));else if(l.type==="iframe"){const a=this.createShaderToyEmbed(l.shaderID,l.originalURL);d.push(a)}}),d.forEach(l=>{n.insertBefore(l,e)}),n.removeChild(e)}}class ct{constructor(){this.init()}init(){const e=document.querySelector(".content");if(!e)return;e.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach(n=>{const i=document.createElement("span");i.className="collapse-button",n.insertBefore(i,n.firstChild),i.addEventListener("click",s=>{s.stopPropagation(),this.toggleCollapse(n)})})}toggleCollapse(e){const t=parseInt(e.tagName[1]);let n=e.nextElementSibling;e.classList.toggle("collapsed");const i=e.classList.contains("collapsed");for(;n&&!(n.tagName&&n.tagName.match(/^H[1-6]$/)&&parseInt(n.tagName[1])<=t);)n.style.display=i?"none":"",n=n.nextElementSibling}}class dt{constructor(){this.init()}init(){this.initCodeBlockExpansion(),new MutationObserver(t=>{let n=!1;t.forEach(i=>{i.addedNodes.length>0&&i.addedNodes.forEach(s=>{s.nodeType===1&&(s.matches("figure.highlight")||s.querySelector("figure.highlight"))&&(n=!0)})}),n&&setTimeout(()=>this.initCodeBlockExpansion(),100)}).observe(document.body,{childList:!0,subtree:!0})}initCodeBlockExpansion(){document.querySelectorAll("figure.highlight").forEach(t=>{if(t.closest(".code-block-container"))return;const n=t.querySelector("table");if(n){const a=n.querySelector("td.code");if(a){const u=document.createElement("pre");u.className="code",u.innerHTML=a.innerHTML,t.innerHTML="",t.appendChild(u)}}const i=t.querySelector("pre.code");if(!i)return;const s=i.scrollHeight,r=400,d=document.createElement("div");d.className="code-buttons";const l=document.createElement("button");if(l.className="copy-code-button",l.textContent="复制代码",l.addEventListener("click",a=>{a.preventDefault(),a.stopPropagation(),this.copyCodeToClipboard(i,l)}),d.appendChild(l),s>r){const a=document.createElement("div");a.className="code-block-container collapsed",t.parentNode.insertBefore(a,t),a.appendChild(t);const u=document.createElement("button");u.className="expand-button",u.textContent="展开代码",d.appendChild(u),a.appendChild(d),u.addEventListener("click",()=>{a.classList.contains("collapsed")&&this.showFullscreenCode(t)})}else{const a=document.createElement("div");a.className="code-block-container",t.parentNode.insertBefore(a,t),a.appendChild(t),a.appendChild(d)}})}showFullscreenCode(e){const t=document.createElement("div");t.className="code-fullscreen-modal active";const n=document.createElement("div");n.className="code-fullscreen-content";const s=(e.closest(".code-block-container")||e).cloneNode(!0);s.querySelectorAll(".code-buttons, .copy-code-button, .expand-button").forEach(p=>{p.parentNode&&p.parentNode.removeChild(p)});const d=s.classList.contains("code-block-container")?s:s.querySelector(".code-block-container");d&&(d.classList.remove("collapsed"),d.style.margin="0");const l=(d||s).querySelector("pre.code");l&&(l.scrollTop=0),n.appendChild(s);const a=document.createElement("button");a.className="close-fullscreen",a.textContent="关闭",n.appendChild(a),t.appendChild(n),document.body.appendChild(t),document.body.style.overflow="hidden";const u=()=>{document.body.removeChild(t),document.body.style.overflow=""};a.addEventListener("click",u),t.addEventListener("click",p=>{p.target===t&&u()});const x=p=>{p.key==="Escape"&&(u(),document.removeEventListener("keydown",x))};document.addEventListener("keydown",x)}copyCodeToClipboard(e,t){const n=e.textContent||e.innerText;navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(n).then(()=>{this.showCopySuccess(t)}).catch(i=>{console.error("复制失败:",i),this.fallbackCopy(n,t)}):this.fallbackCopy(n,t)}fallbackCopy(e,t){const n=document.createElement("textarea");n.value=e,n.style.position="fixed",n.style.top="0",n.style.left="0",n.style.width="2em",n.style.height="2em",n.style.padding="0",n.style.border="none",n.style.outline="none",n.style.boxShadow="none",n.style.background="transparent",document.body.appendChild(n),n.focus(),n.select();try{document.execCommand("copy")&&this.showCopySuccess(t)}catch(i){console.error("复制失败:",i)}document.body.removeChild(n)}showCopySuccess(e){const t=e.textContent;e.classList.add("copied"),e.textContent="已复制 ✓",setTimeout(()=>{e.classList.remove("copied"),e.textContent=t},2e3)}}const Se=1.2,Le=1.15,ht=.2,ft=50,ut="canvas-arrow-modal-";let Ce=0;class pt{constructor(){const e=document.querySelectorAll(".canvas-embed:not(.canvas-embed--error)");if(e.length)for(const t of e)this.attach(t)}attach(e){e.setAttribute("role","button"),e.setAttribute("tabindex","0"),e.setAttribute("aria-label","点击放大查看画布"),e.addEventListener("click",t=>{t.target.closest("a")||(t.preventDefault(),this.openModal(e))}),e.addEventListener("keydown",t=>{(t.key==="Enter"||t.key===" ")&&(t.preventDefault(),this.openModal(e))})}openModal(e){const t=e.querySelector(".canvas-svg");if(!t)return;const n=t.cloneNode(!0);mt(n),n.classList.add("canvas-modal__svg");const i=document.createElement("div");i.className="canvas-modal",i.innerHTML=`
    ...
    `,i.querySelector(".canvas-modal__viewport").appendChild(n),document.body.appendChild(i),document.body.classList.add("canvas-modal-open");const s=new vt(n),r=l=>{l.key==="Escape"&&d()},d=()=>{s.destroy(),i.remove(),document.body.classList.remove("canvas-modal-open"),document.removeEventListener("keydown",r)};i.querySelector(".canvas-modal__close").addEventListener("click",d),i.querySelector(".canvas-modal__overlay").addEventListener("click",d),document.addEventListener("keydown",r),i.querySelectorAll(".canvas-modal__btn").forEach(l=>{l.addEventListener("click",()=>{const a=l.dataset.action;a==="zoom-in"?s.zoomBy(Se):a==="zoom-out"?s.zoomBy(1/Se):a==="reset"&&s.reset()})})}}function mt(o){const e=o.querySelector("#canvas-arrow");if(!e)return;Ce+=1;const t=`${ut}${Ce}`;e.id=t,o.querySelectorAll("[marker-end]").forEach(n=>{n.setAttribute("marker-end",`url(#${t})`)})}class vt{constructor(e){this.svg=e;const t=e.viewBox.baseVal;this.original={x:t.x,y:t.y,w:t.width,h:t.height},this.state={...this.original},this.pointers=new Map,this.pinch=null,this.svg.style.cursor="grab",this.svg.style.touchAction="none",this.onWheel=this.onWheel.bind(this),this.onPointerDown=this.onPointerDown.bind(this),this.onPointerMove=this.onPointerMove.bind(this),this.onPointerUp=this.onPointerUp.bind(this),this.svg.addEventListener("wheel",this.onWheel,{passive:!1}),this.svg.addEventListener("pointerdown",this.onPointerDown),this.svg.addEventListener("pointermove",this.onPointerMove),this.svg.addEventListener("pointerup",this.onPointerUp),this.svg.addEventListener("pointercancel",this.onPointerUp)}setViewBox(){const{x:e,y:t,w:n,h:i}=this.state;this.svg.setAttribute("viewBox",`${e} ${t} ${n} ${i}`)}currentScale(){return this.original.w/this.state.w}zoomBy(e,t,n){const i=this.currentScale()*e;i<ht||i>ft||(t==null&&(t=this.state.x+this.state.w/2),n==null&&(n=this.state.y+this.state.h/2),this.state.x=t-(t-this.state.x)/e,this.state.y=n-(n-this.state.y)/e,this.state.w/=e,this.state.h/=e,this.setViewBox())}pan(e,t){this.state.x-=e,this.state.y-=t,this.setViewBox()}reset(){this.state={...this.original},this.setViewBox()}screenToSvg(e,t){const n=this.svg.createSVGPoint();n.x=e,n.y=t;const i=this.svg.getScreenCTM();return i?n.matrixTransform(i.inverse()):{x:0,y:0}}onWheel(e){e.preventDefault();const t=e.deltaY<0?Le:1/Le,{x:n,y:i}=this.screenToSvg(e.clientX,e.clientY);this.zoomBy(t,n,i)}onPointerDown(e){e.target.closest("a")||(this.svg.setPointerCapture(e.pointerId),this.pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY,svg:this.screenToSvg(e.clientX,e.clientY)}),this.pointers.size===2?this.pinch=this.computePinch():this.pointers.size===1&&(this.svg.style.cursor="grabbing"))}onPointerMove(e){const t=this.pointers.get(e.pointerId);if(t){if(t.clientX=e.clientX,t.clientY=e.clientY,this.pointers.size===2&&this.pinch){const n=this.computePinch(),i=n.dist/this.pinch.dist;if(i>0&&Number.isFinite(i)){const s=this.screenToSvg(n.cx,n.cy);this.zoomBy(i,s.x,s.y)}this.pinch=n}else if(this.pointers.size===1){const n=this.screenToSvg(e.clientX,e.clientY);this.pan(n.x-t.svg.x,n.y-t.svg.y)}}}onPointerUp(e){this.pointers.delete(e.pointerId),this.pointers.size<2&&(this.pinch=null),this.pointers.size===0&&(this.svg.style.cursor="grab")}computePinch(){const[e,t]=[...this.pointers.values()],n=t.clientX-e.clientX,i=t.clientY-e.clientY;return{dist:Math.hypot(n,i),cx:(e.clientX+t.clientX)/2,cy:(e.clientY+t.clientY)/2}}destroy(){this.svg.removeEventListener("wheel",this.onWheel),this.svg.removeEventListener("pointerdown",this.onPointerDown),this.svg.removeEventListener("pointermove",this.onPointerMove),this.svg.removeEventListener("pointerup",this.onPointerUp),this.svg.removeEventListener("pointercancel",this.onPointerUp)}}const de={en:{Home:"Home",Daily:"Daily",Archives:"Archives",About:"About",Portfolio:"Portfolio",Bilibili:"Bilibili",GitHub:"GitHub",Instagram:"Instagram",Douban:"Douban",Email:"Email",RSS:"RSS",Language:"Language",Copyright:"Copyright","Powered by":"Powered by","Modified based on":"Modified based on",theme:"theme","Mainly maintained using AI":"Mainly maintained using AI","Older Posts":"Older Posts","Newer Posts":"Newer Posts",Comments:"Comments","Switch to Chinese":"Switch to Chinese","Switch to English":"Switch to English","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"Switched to English",articleLanguageSwitched:"Switched to English version","tag-graph-hint":"Click tag to explore · Scroll to zoom · Drag to rearrange"},"zh-CN":{Home:"首页",Daily:"资讯",Archives:"归档",About:"关于",Portfolio:"作品集",Bilibili:"哔哩哔哩",GitHub:"GitHub",Instagram:"Instagram",Douban:"豆瓣",Email:"邮箱",RSS:"RSS",Language:"语言",Copyright:"版权所有","Powered by":"技术支持","Modified based on":"基于",theme:"主题","Mainly maintained using AI":"主要使用AI来维护","Older Posts":"上一页","Newer Posts":"下一页",Comments:"留言","Switch to Chinese":"切换至中文","Switch to English":"切换至英文","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"已切换至中文",articleLanguageSwitched:"已切换至中文版本","tag-graph-hint":"点击标签探索 · 滚轮缩放 · 拖拽移动"}},gt=()=>(navigator.language||navigator.userLanguage).startsWith("zh")?"zh-CN":"en",Ae=()=>localStorage.getItem("siteLanguage")||gt(),yt=()=>{const o=document.cookie.match(/(?:^|;\s*)lang_pref=([^;]+)/);return o?o[1]:null},wt=o=>{document.cookie="lang_pref="+o+"; path=/; max-age=31536000; samesite=lax"},Ie=()=>{const o=document.querySelector('meta[name="article:lang"]');return o?o.content:window.location.pathname.includes(".zh-CN")?"zh-CN":"en"},Re=o=>{const e=document.querySelector(`link[rel="alternate"][hreflang="${o}"]`);if(!e)return null;const t=new URL(e.href,window.location.origin);return window.location.origin+t.pathname+t.search+t.hash},ze=o=>{document.querySelectorAll(".lang-switch__opt").forEach(e=>{e.setAttribute("aria-pressed",e.getAttribute("data-lang")===o?"true":"false")})},he=o=>{const e=de[o];if(!e){console.warn("Language data not available for:",o);return}document.documentElement.lang=o,document.querySelectorAll("nav ul li a").forEach(l=>{const a=l.getAttribute("data-i18n-key");a&&e[a]&&(l.textContent=e[a])}),document.querySelectorAll("[data-i18n]").forEach(l=>{const a=l.getAttribute("data-i18n");e[a]&&(l.textContent=e[a])}),document.querySelectorAll("[data-title]").forEach(l=>{const a=l.getAttribute("data-title");e[a]&&l.setAttribute("data-title",e[a])});const s=document.querySelector(".pagination .extend.prev"),r=document.querySelector(".pagination .extend.next");s&&(s.textContent=e["Older Posts"]||s.textContent),r&&(r.textContent=e["Newer Posts"]||r.textContent),localStorage.setItem("siteLanguage",o),document.querySelectorAll("[data-i18n-tag]").forEach(l=>{const a=l.getAttribute("data-i18n-tag");if(o==="zh-CN"){const u=window.tagTranslations&&window.tagTranslations[a];u&&(l.textContent=u)}else l.textContent=a}),ze(o)},bt=o=>{const e=document.querySelector(".lang-notification");e&&e.remove();const t=document.createElement("div");t.className="lang-notification",t.textContent=o,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.parentNode&&t.parentNode.removeChild(t)},300)},2e3)},Ne=o=>{if(wt(o),o===Ie()){localStorage.setItem("siteLanguage",o),he(o);return}const e=Re(o);if(e){localStorage.setItem("siteLanguage",o),window.location.href=e;return}he(o);const t=de[o]?de[o].languageSwitched:"Language switched";bt(t)},xt=()=>{const o=Ae()==="zh-CN"?"en":"zh-CN";Ne(o)},Et=()=>{document.querySelectorAll(".lang-switch__opt").forEach(o=>{o.addEventListener("click",e=>{e.preventDefault(),Ne(o.getAttribute("data-lang"))})}),ze(Ae())},ke=()=>{const o=Ie(),e=yt();if(he(e||o),e&&e!==o){const t=Re(e),n=t&&new URL(t,window.location.origin).pathname;n&&n!==window.location.pathname&&window.location.replace(t)}};function St(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ke):ke(),window.addEventListener("load",()=>{const o=document.getElementById("langSwitch");o&&o.addEventListener("click",e=>{e.preventDefault(),xt()}),Et()})}function Lt(){const o=document.getElementById("tag-graph"),e=document.getElementById("tag-graph-container"),t=window.__TAG_GRAPH_DATA__;if(!o||!t||!t.nodes||t.nodes.length===0)return;const n=document.createElement("div");n.className="graph-loading",n.textContent="Loading",o.parentNode.appendChild(n);const i=t.archiveFilterTags||[],s={};i.forEach(function(c){s[c]=!0});function r(c){return String(c||"").replace(/-/g," ").replace(/\s+/g," ").trim().toLowerCase()}const d={};t.links.forEach(function(c){d[c.source]=(d[c.source]||0)+1,d[c.target]=(d[c.target]||0)+1});let l=1;t.nodes.forEach(function(c){const h=c.value||0;h>l&&(l=h)}),t.nodes.forEach(function(c){const h=c.value||0;c.symbolSize=Math.max(12,Math.min(70,12+h*(58/l)))});const a={};t.nodes.forEach(function(c){a[c.name]=[]});function u(c,h){!a[c]||!a[h]||c===h||(a[c].indexOf(h)===-1&&a[c].push(h),a[h].indexOf(c)===-1&&a[h].push(c))}t.links.forEach(function(c){u(c.source,c.target)});const x={};t.nodes.forEach(function(c){const h=r(c.name);x[h]||(x[h]=[]),x[h].push(c.name)}),Object.keys(x).forEach(function(c){const h=x[c];if(!(h.length<2))for(let g=0;g<h.length;g++)for(let S=g+1;S<h.length;S++)u(h[g],h[S])});const p={},k=[];i.forEach(function(c){a[c]!==void 0&&(p[c]=0,k.push(c))});let R=0;for(;R<k.length;){const c=k[R++];(a[c]||[]).forEach(function(h){p[h]===void 0&&(p[h]=p[c]+1,k.push(h))})}Object.keys(p).forEach(function(c){});const I=[{h:260,s:62,l:50},{h:15,s:80,l:55},{h:160,s:60,l:42},{h:220,s:72,l:52},{h:340,s:70,l:52},{h:45,s:85,l:50},{h:190,s:70,l:45},{h:90,s:55,l:45},{h:290,s:60,l:50},{h:30,s:75,l:48},{h:130,s:50,l:42},{h:0,s:70,l:55}];function U(c,h,g){return"hsl("+Math.round(c)+", "+Math.round(h)+"%, "+Math.round(g)+"%)"}const O={},m={},w=t.nodes.filter(function(c){return s[c.name]});w.sort(function(c,h){return h.value-c.value}),w.forEach(function(c,h){const g=I[h%I.length];m[c.name]=g,O[c.name]=U(g.h,g.s,g.l)});const E={};i.forEach(function(c){if(a[c]===void 0)return;const h={};h[c]=0;const g=[c];let S=0;for(;S<g.length;){const z=g[S++];(a[z]||[]).forEach(function(M){h[M]===void 0&&(h[M]=h[z]+1,g.push(M))})}E[c]=h});const F=t.nodes.filter(function(c){return!s[c.name]});let Q=1;F.forEach(function(c){let h=1/0;i.forEach(function(g){if(!E[g])return;const S=E[g][c.name];S!==void 0&&S<h&&(h=S)}),h<1/0&&h>Q&&(Q=h)}),F.forEach(function(c){const h=[];let g=0;if(i.forEach(function(D){if(!E[D]||!m[D])return;let P=E[D][c.name];if(P===void 0)return;P===0&&(P=.5);const W=1/(P*P);h.push({ft:D,w:W}),g+=W}),g===0){O[c.name]="hsl(0, 0%, 82%)";return}let S=0,z=0,M=0,T=0;h.forEach(function(D){const P=D.w/g,W=m[D.ft],ne=W.h*Math.PI/180;S+=Math.sin(ne)*P,z+=Math.cos(ne)*P,M+=W.s*P,T+=W.l*P});let A=Math.atan2(S,z)*180/Math.PI;A<0&&(A+=360);let H=M,_=T,te=1/0;h.forEach(function(D){const P=E[D.ft][c.name];P<te&&(te=P)});let K=(te-1)/Math.max(Q-1,1);K=Math.max(0,Math.min(1,K));const oe=Math.pow(K,.85),ce=32,G=1-oe*.35;H=Math.max(ce,H*G),_=_+oe*(82-_)*.78,O[c.name]=U(A,H,_)});const j=t.tagTranslations||{},ie={};Object.keys(j).forEach(function(c){ie[c]=j[c];const h=c.replace(/-/g," ");h!==c&&(ie[h]=j[c])});function se(c){return(typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en")==="zh-CN"&&ie[c]?ie[c]:c}const pe=e.getBoundingClientRect(),V=pe.width||500,Y=pe.height||400,me=Math.min(V,Y)*.28;w.forEach(function(c,h){const g=2*Math.PI*h/Math.max(w.length,1)-Math.PI/2;c.x=V/2+me*Math.cos(g),c.y=Y/2+me*Math.sin(g)});const ve=Math.min(V,Y)*.45;F.forEach(function(c,h){const g=2*Math.PI*h/Math.max(F.length,1);c.x=V/2+ve*Math.cos(g),c.y=Y/2+ve*Math.sin(g)});const _e=60,q=t.nodes;for(let c=0;c<15;c++)for(let h=0;h<q.length;h++)for(let g=h+1;g<q.length;g++){const S=q[g].x-q[h].x,z=q[g].y-q[h].y,M=Math.sqrt(S*S+z*z),T=_e+(q[h].symbolSize+q[g].symbolSize)/2;if(M<T){const A=(T-M)/2,H=M>.1?S/M:Math.random()-.5,_=M>.1?z/M:Math.random()-.5;q[h].x-=H*A,q[h].y-=_*A,q[g].x+=H*A,q[g].y+=_*A}}let J=1,ee=[V/2,Y/2];if(w.length>0){let c=1/0,h=-1/0,g=1/0,S=-1/0;w.forEach(function(T){const A=(T.symbolSize||20)/2+50;T.x-A<c&&(c=T.x-A),T.x+A>h&&(h=T.x+A),T.y-A<g&&(g=T.y-A),T.y+A>S&&(S=T.y+A)});const z=h-c,M=S-g;if(z>0&&M>0){const T=V/z,A=Y/M;J=Math.min(T,A,1.5)*.8,J<.3&&(J=.3),ee=[(c+h)/2,(g+S)/2]}}t.nodes.forEach(function(c){c.itemStyle={color:O[c.name],borderColor:"#fff",borderWidth:1.5,shadowBlur:5,shadowColor:"rgba(0, 0, 0, 0.06)"},c.label={show:!0,formatter:function(){return se(c.name)},fontSize:Math.max(10,Math.min(15,9+(d[c.name]||0)*.5)),color:"#555",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'}});const X=e.querySelector(".tag-graph-hint");let ge=null,le=!1;function Oe(){le||(le=!0,X.classList.add("visible"),clearTimeout(ge),ge=setTimeout(function(){X.classList.remove("visible"),le=!1},3e3))}e.addEventListener("mouseenter",Oe);const $=document.createElement("script");$.src="https://cdn.jsdelivr.net/npm/echarts@5.5.1/dist/echarts.min.js",$.integrity="sha384-Mx5lkUEQPM1pOJCwFtUICyX45KNojXbkWdYhkKUKsbv391mavbfoAmONbzkgYPzR",$.crossOrigin="anonymous",$.onload=function(){He()},$.onerror=function(){n.textContent="Failed to load chart library",n.style.color="#c44"},document.head.appendChild($);function Fe(c){return c<10?750:c<20?1200:c<40?1650:2100}function He(){n.parentNode&&n.parentNode.removeChild(n);const c=window.matchMedia&&window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window,h=echarts.init(o),g={backgroundColor:"transparent",tooltip:{show:!0,enterable:!0,confine:!0,backgroundColor:"rgba(255, 255, 255, 0.97)",borderColor:"#e8e8e8",borderWidth:1,padding:[10,14],textStyle:{color:"#4b4848",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',fontSize:13},extraCssText:"border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); max-height: 260px; overflow-y: auto;",formatter:function(f){function v(C){return String(C??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function L(C,b){const y=v(C),N=v(b),B='style="display:block;color:#666;font-size:11px;line-height:1.6;padding:1px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px;text-decoration:none;"';return N?'<a href="'+N+'" '+B+">• "+y+"</a>":"<div "+B+">• "+y+"</div>"}if(f.dataType==="node"){const C=se(f.name);let b='<div style="font-weight:600;font-size:14px;margin-bottom:5px;color:'+(O[f.name]||"#795da3")+'">'+v(C)+"</div>";b+='<div style="color:#777;font-size:12px;margin-bottom:6px;">📄 '+f.value+" article"+(f.value>1?"s":"")+"</div>";const y=t.postTitles&&t.postTitles[f.name];return y&&y.length>0&&(b+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;">',y.forEach(function(N){typeof N=="string"?b+=L(N,""):b+=L(N.title,N.path)}),b+="</div>"),b}if(f.dataType==="edge"){const C=f.data.source,b=f.data.target;let y='<span style="font-weight:600">'+v(se(C))+'</span> <span style="color:#bbb">↔</span> <span style="font-weight:600">'+v(se(b))+"</span>";y+='<br/><span style="color:#999;font-size:12px">📄 '+f.data.value+" article"+(f.data.value>1?"s":"")+"</span>";const N=[C,b].sort().join("	"),B=t.linkPosts&&t.linkPosts[N];return B&&B.length>0&&(y+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;margin-top:5px;">',B.forEach(function(ae){y+=L(ae.title,ae.path)}),y+="</div>"),y}}},animationDuration:1500,animationEasingUpdate:"quinticInOut",series:[{type:"graph",layout:"force",data:t.nodes,links:t.links,roam:!c,draggable:!1,force:{repulsion:Fe(t.nodes.length),edgeLength:[150,450],gravity:.12,friction:.6,layoutAnimation:!0},emphasis:{focus:"adjacency",blurScope:"global",itemStyle:{shadowBlur:20,shadowColor:"rgba(121, 93, 163, 0.45)",borderWidth:2,borderColor:"#fff"},lineStyle:{width:3,opacity:.85},label:{show:!0,fontSize:14,fontWeight:"bold",color:"#333"}},label:{position:"right",distance:6},lineStyle:{color:"#d0d0d0",width:1.5,curveness:0,opacity:.35},scaleLimit:{min:.3,max:4},zoom:J,center:ee}]};let S=J||1,z=ee?[ee[0],ee[1]]:[0,0],M=!1,T=null;h.setOption(g);let A=null,H=null;function _(){const f=h.getModel();return f&&f.getSeriesByIndex&&f.getSeriesByIndex(0)}function te(){const f=_();return f&&f.getGraph&&f.getGraph()}function K(f){const v=[];for(let L=0;L<t.nodes.length;L++){const C=t.nodes[L],b=f&&f.getNodeById&&f.getNodeById(C.name),y=b&&b.getLayout&&b.getLayout();if(!y||y.length<2||!Number.isFinite(y[0])||!Number.isFinite(y[1]))return null;v.push(Object.assign({},C,{x:y[0],y:y[1]}))}return v}function oe(f){let v=1/0,L=-1/0,C=1/0,b=-1/0;return f.forEach(function(y){y.x<v&&(v=y.x),y.x>L&&(L=y.x),y.y<C&&(C=y.y),y.y>b&&(b=y.y)}),L===v&&(v-=1,L+=1),b===C&&(C-=1,b+=1),{width:L-v,height:b-C}}function ce(f){const v=oe(f),L=h.getWidth()/T.chartWidth,C=h.getHeight()/T.chartHeight,b=Math.min(L,C),y=T.rawScale*b,N=v.width*y,B=v.height*y,ae=T.anchorX*h.getWidth(),Be=T.anchorY*h.getHeight();return{left:ae-N/2,top:Be-B/2,width:N,height:B}}function G(f){const v=ce(f);h.setOption({series:[{animation:!1,layout:"none",data:f,draggable:!c,roam:!c,left:v.left,top:v.top,width:v.width,height:v.height,center:z.slice(),zoom:S}]})}function D(){if(M)return;const f=_(),v=f&&f.coordinateSystem,L=f&&f.getGraph&&f.getGraph(),C=K(L),b=v&&v.getTransformInfo&&v.getTransformInfo(),y=b&&b.raw,N=v&&v.getViewRect&&v.getViewRect();if(!C||!y||!N){A=setTimeout(D,100);return}S=v.getZoom?v.getZoom():S,z=v.getCenter?v.getCenter().slice():z,T={chartWidth:h.getWidth(),chartHeight:h.getHeight(),rawScale:Math.sqrt(Math.abs(y.scaleX*y.scaleY)),anchorX:(N.x+N.width/2)/h.getWidth(),anchorY:(N.y+N.height/2)/h.getHeight()},t.nodes=C,M=!0,clearTimeout(A),clearTimeout(H),h.off("rendered",P),G(t.nodes)}function P(){M||(clearTimeout(A),A=setTimeout(D,200))}h.on("rendered",P),h.on("graphroam",function(){const f=_(),v=f&&f.get&&f.get("zoom"),L=f&&f.get&&f.get("center");Number.isFinite(v)&&(S=v),L&&L.length>=2&&(z=[L[0],L[1]])}),H=setTimeout(D,2500),h.on("click",function(f){f.dataType==="node"&&t.tagPaths&&t.tagPaths[f.name]&&(window.location.href=t.tagPaths[f.name])}),h.on("mouseover",function(f){(f.dataType==="node"||f.dataType==="edge")&&(o.style.cursor="pointer")}),h.on("mouseout",function(){o.style.cursor="default"});function W(){if(!M)return;const f=K(te());!f||!f.some(function(L,C){const b=t.nodes[C];return!b||L.x!==b.x||L.y!==b.y})||(t.nodes=f,G(t.nodes))}const ne=h.getZr();if(ne.on("mouseup",function(){requestAnimationFrame(W)}),ne.on("globalout",function(){requestAnimationFrame(W)}),c){const f=document.createElement("button");f.type="button",f.className="tag-graph-fs-btn",f.setAttribute("aria-label","Fullscreen"),f.innerHTML='<svg class="tag-graph-fs-ic tag-graph-fs-ic--open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg><svg class="tag-graph-fs-ic tag-graph-fs-ic--close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',e.appendChild(f);let v=!1;const L=function(){if(!X)return;const y=typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en";X.textContent=y==="zh-CN"?"双指缩放 · 拖动平移 · 点按进入标签":"Pinch to zoom · Drag to pan · Tap a tag",X.classList.add("visible"),setTimeout(function(){X.classList.remove("visible")},2600)},C=function(){v=!0,e.classList.add("tag-graph-fullscreen"),f.classList.add("is-fullscreen"),f.setAttribute("aria-label","Exit fullscreen"),document.body.style.overflow="hidden",document.body.classList.add("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),M&&G(t.nodes),h.setOption({series:[{roam:!0}]}),L()})},b=function(){v=!1,e.classList.remove("tag-graph-fullscreen"),f.classList.remove("is-fullscreen"),f.setAttribute("aria-label","Fullscreen"),document.body.style.overflow="",document.body.classList.remove("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),M&&G(t.nodes),h.setOption({series:[{roam:!1,zoom:S,center:z.slice()}]})})};f.addEventListener("click",function(y){y.preventDefault(),y.stopPropagation(),v?b():C()}),document.addEventListener("keydown",function(y){y.key==="Escape"&&v&&b()})}let ye;window.addEventListener("resize",function(){clearTimeout(ye),ye=setTimeout(function(){h.resize(),M&&G(t.nodes)},150)});function we(){M?G(t.nodes):h.setOption({series:[{data:t.nodes}]})}window.addEventListener("storage",function(f){f.key==="siteLanguage"&&we()});const We=localStorage.setItem;localStorage.setItem=function(f,v){We.call(localStorage,f,v),f==="siteLanguage"&&setTimeout(we,50)}}}function Ct(o,e){let t=0,n="";for(const i of o){const s=i.codePointAt(0),r=s>=12288&&s<=12351||s>=13312&&s<=40959||s>=65280&&s<=65519;if(t+=r?1:.5,t>e)return n.replace(/\s+$/,"")+"…";n+=i}return n}function kt(o,e,t={}){let n=(o||"").trim();const i=n.search(/[（(]/);i>4&&(n=n.slice(0,i));const s=n.indexOf(" · ");if(s>0&&(n=n.slice(0,s)),e!=="quote"&&e!=="cite"){const r=n.indexOf(" — ");r>0&&(n=n.slice(0,r))}if(n=n.trim(),t.short){const r=n.search(/[，,]/);r>1&&(n=n.slice(0,r)),n=Ct(n,15)}return n}function Mt(o){if(o.querySelector("audio"))return!1;const e=o.querySelector("summary");return!(!e||/跟读|本节语音/.test(e.textContent||"")||o.parentElement&&o.parentElement.closest("details.callout"))}function Tt(o,e={}){const t=e.includeCallouts?"h1, h2, h3, h4, h5, h6, details.callout--foldable":"h1, h2, h3, h4, h5, h6",n=Array.from(o.querySelectorAll(t)),i=[];let s=1;const r=[];return n.forEach(d=>{const l=/^H[1-6]$/.test(d.tagName);let a,u,x=!1;if(l)a=parseInt(d.tagName[1],10),s=a,u=d.textContent;else{if(!Mt(d))return;a=s+1,x=!0;const I=d.querySelector("summary");if(u=kt(I.textContent||"",d.getAttribute("data-callout")||"",{short:!0}),!u)return}let p=1;for(;i.length&&i[i.length-1].level>=a;){const I=i.pop();I.level===a&&(p=I.n+1)}i.push({level:a,n:p});const k=r.length;d.id||(d.id=x?`toc-item-${k}`:`heading-${k}`);const R=i.map(I=>I.n).join(".");d.dataset.tocNumber=R,r.push({element:d,level:a,index:k,id:d.id,text:u,number:R,virtual:x})}),r}function At(o){const e=document.createElement("aside");e.className="toc-drawer",e.innerHTML='<div class="toc-header"><span class="toc-title"></span><div class="toc-header__actions"><button type="button" class="toc-foldall"></button><button type="button" class="toc-close-btn"><span class="toc-close-btn__x" aria-hidden="true"></span></button></div></div><div class="toc-content"><div class="toc-list"></div></div>',document.body.appendChild(e);const t=e.querySelector(".toc-list"),n=o.map(i=>{const s=document.createElement("div");if(s.className=i.virtual?"toc-item toc-item--virtual":"toc-item",s.setAttribute("data-level",String(i.level)),s.setAttribute("data-index",String(i.index)),!i.virtual){const d=document.createElement("div");d.className="toc-collapse-btn",s.appendChild(d)}const r=document.createElement("span");return r.className="toc-item-text",r.style.cursor="pointer",r.innerHTML=`<span class="toc-number">${i.number}.</span> `,r.appendChild(document.createTextNode(i.text)),r.setAttribute("title",i.text),i.element.classList.contains("collapsed")&&s.classList.add("collapsed"),s.appendChild(r),t.appendChild(s),s});return{container:e,items:n}}const Pe="toc-panel-state";function De(){try{const o=localStorage.getItem(Pe);if(!o)return null;const e=JSON.parse(o);return!e||typeof e!="object"?null:e}catch{return null}}function It(o){try{const t={...De()||{},...o};localStorage.setItem(Pe,JSON.stringify(t))}catch{}}function Rt(){return(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?"zh":"en"}const zt={zh:{title:"目录",open:"目录",hide:"收起目录",show:"打开目录"},en:{title:"Contents",open:"TOC",hide:"Hide contents",show:"Show contents"}};function Nt(o){const e=o.querySelector(".toc-content"),t=o.querySelector(".toc-item.toc-reading");!e||!t||(e.scrollTop=t.offsetTop-e.clientHeight/2+t.offsetHeight/2)}function Pt(o){const e=zt[Rt()],t=o.querySelector(".toc-title");t&&(t.textContent=e.title),o.setAttribute("aria-label",e.title);const n=o.querySelector(".toc-close-btn");n&&(n.setAttribute("aria-label",e.hide),n.setAttribute("title",e.hide));const i=document.createElement("button");i.type="button",i.className="toc-tab",i.setAttribute("aria-label",e.show),i.innerHTML='<span class="toc-tab__icon" aria-hidden="true"></span><span class="toc-tab__text">'+e.open+"</span>",document.body.appendChild(i);const s=document.createElement("div");s.className="toc-scrim",document.body.appendChild(s);function r(a,u){o.classList.toggle("is-open",a),i.classList.toggle("is-hidden",a),s.classList.toggle("is-visible",a),document.body.classList.toggle("toc-drawer-open",a),a&&Nt(o),u&&It({hidden:!a})}i.addEventListener("click",()=>r(!0,!0)),s.addEventListener("click",()=>r(!1,!0)),n&&n.addEventListener("click",a=>{a.stopPropagation(),r(!1,!0)}),document.addEventListener("keydown",a=>{a.key==="Escape"&&o.classList.contains("is-open")&&r(!1,!1)});const d=De(),l=window.matchMedia("(min-width: 1100px)").matches;return r(l&&!!d&&d.hidden===!1,!1),{setOpen:r}}function ue(o,e,t){for(let n=e+1;n<o.length&&!(parseInt(o[n].getAttribute("data-level")||"1",10)<=t);n+=1)o[n].classList.add("toc-hidden")}function qe(o,e,t){for(let n=e+1;n<o.length;n+=1){const i=parseInt(o[n].getAttribute("data-level")||"1",10);if(i<=t)break;if(i===t+1)o[n].classList.remove("toc-hidden");else{let s=!0;for(let r=n-1;r>e;r-=1){const d=parseInt(o[r].getAttribute("data-level")||"1",10);if(d<i&&o[r].classList.contains("collapsed")){s=!1;break}if(d<=t)break}s&&o[n].classList.remove("toc-hidden")}}}function Dt(o){const e=parseInt(o.tagName.charAt(1),10);let t=o.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t=t.nextElementSibling;continue}t.style.display="none",t=t.nextElementSibling}}function qt(o){const e=parseInt(o.tagName.charAt(1),10);let t=o.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t.style.display="",t=t.nextElementSibling;continue}t.style.display="",t=t.nextElementSibling}}function fe(o,e,t){const n=e[t],i=o[t]&&o[t].element;if(!n||!i)return;const s=parseInt(n.getAttribute("data-level")||"1",10);!n.classList.contains("collapsed")?(n.classList.add("collapsed"),ue(e,t,s),i.classList.contains("collapsed")||(i.classList.add("collapsed"),Dt(i))):(n.classList.remove("collapsed"),qe(e,t,s),i.classList.contains("collapsed")&&(i.classList.remove("collapsed"),qt(i)))}function _t(o,e,t){e.forEach((n,i)=>{if(n.classList.contains("toc-item--virtual")||!n.querySelector(".toc-collapse-btn"))return;const s=n.classList.contains("collapsed");t?!s&&!n.classList.contains("toc-hidden")&&fe(o,e,i):s&&fe(o,e,i)})}function Ot(o,e,t){const n=e[t],i=o[t]&&o[t].element;if(!n||!i)return;const s=parseInt(n.getAttribute("data-level")||"1",10),r=i.classList.contains("collapsed");r&&!n.classList.contains("collapsed")?(n.classList.add("collapsed"),ue(e,t,s)):!r&&n.classList.contains("collapsed")&&(n.classList.remove("collapsed"),qe(e,t,s))}function Ft(o,e){e.forEach((i,s)=>{if(i.classList.contains("collapsed")){const r=parseInt(i.getAttribute("data-level")||"1",10);ue(e,s,r)}}),e.forEach((i,s)=>{const r=i.querySelector(".toc-collapse-btn");r&&r.addEventListener("click",d=>{d.stopPropagation(),fe(o,e,s)})});const t=new Map;o.forEach((i,s)=>t.set(i.element,s));const n=new MutationObserver(i=>{i.forEach(s=>{if(s.type!=="attributes"||s.attributeName!=="class")return;const r=t.get(s.target);r!==void 0&&Ot(o,e,r)})});return o.forEach(i=>{n.observe(i.element,{attributes:!0,attributeFilter:["class"]})}),{observer:n}}const Me={1:{passed:"rgba(128,128,128,0.1)",reading:"rgba(66,153,225,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(66,153,225,0.25)"},2:{passed:"rgba(128,128,128,0.1)",reading:"rgba(49,130,206,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(49,130,206,0.25)"},3:{passed:"rgba(128,128,128,0.1)",reading:"rgba(44,82,130,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(44,82,130,0.25)"},4:{passed:"rgba(128,128,128,0.1)",reading:"rgba(42,67,101,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(42,67,101,0.25)"},5:{passed:"rgba(128,128,128,0.1)",reading:"rgba(26,54,93,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(26,54,93,0.25)"},6:{passed:"rgba(128,128,128,0.1)",reading:"rgba(21,62,117,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(21,62,117,0.25)"}};function Ht(o,e){if(!o.length)return{destroy(){}};const t=new Array(o.length).fill("coming"),n=new Set;let i=-1;const s=new Map;o.forEach((p,k)=>s.set(p.element,k));function r(){e.forEach((p,k)=>{const R=parseInt(p.getAttribute("data-level")||"1",10),I=Me[R]||Me[1],U=t[k];p.classList.remove("toc-passed","toc-reading","toc-coming"),p.style.boxShadow="",p.style.transform="",p.style.fontWeight="",k===i?(p.classList.add("toc-reading"),p.style.backgroundColor=I.active,p.style.opacity="1",p.style.fontWeight="600",p.style.boxShadow="inset 0 0 0 2px rgba(66,153,225,0.3)",p.style.transform="scale(1.02)",p.style.transition="all 0.2s ease"):U==="reading"?(p.classList.add("toc-reading"),p.style.backgroundColor=I.reading,p.style.opacity="1",p.style.fontWeight="600"):U==="passed"?(p.classList.add("toc-passed"),p.style.backgroundColor=I.passed,p.style.opacity="0.7"):(p.classList.add("toc-coming"),p.style.backgroundColor=I.coming,p.style.opacity="0.5")})}function d(){const p=window.innerHeight/2;let k=-1;n.forEach(R=>{const I=o[R].element.getBoundingClientRect();I.top<=p&&I.bottom>=p&&(k=R)}),k!==i&&(i=k,r())}let l=null;function a(){l||(l=requestAnimationFrame(()=>{l=null,d()}))}const u=new IntersectionObserver(p=>{p.forEach(k=>{const R=s.get(k.target);R!==void 0&&(k.isIntersecting?(n.add(R),t[R]="reading"):(n.delete(R),t[R]=k.boundingClientRect.bottom<0?"passed":"coming"))}),d(),r()});o.forEach(p=>u.observe(p.element)),window.addEventListener("scroll",a,{passive:!0}),window.addEventListener("resize",a,{passive:!0}),r();function x(){u.disconnect(),window.removeEventListener("scroll",a),window.removeEventListener("resize",a),l&&cancelAnimationFrame(l)}return{destroy:x,refresh:()=>{d(),r()}}}const Wt='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M7.41 18.59 8.83 20 12 16.83 15.17 20l1.41-1.41L12 14zM16.59 5.41 15.17 4 12 7.17 8.83 4 7.41 5.41 12 10z"/></svg>',Bt='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 5.83 15.17 9l1.41-1.41L12 3 7.42 7.59 8.83 9zm0 12.34L8.83 15l-1.41 1.4L12 21l4.58-4.6L15.17 15z"/></svg>';function Ut(o){return!o.classList.contains("toc-item--virtual")&&!!o.querySelector(".toc-collapse-btn")}function jt(){const o=document.querySelector(".content");if(!o||o.classList.contains("archives"))return null;const e=document.body;if(e.classList.contains("path-about-index-html")||e.classList.contains("layout-about"))return null;const t=document.querySelector("section.main[data-toc]");return t&&t.getAttribute("data-toc")==="false"?null:o}function Gt(){const o=jt();if(!o)return;const e=Tt(o,{includeCallouts:document.body.classList.contains("type-daily-feed")});if(!e.length)return;const{container:t,items:n}=At(e);Ft(e,n);const i=Ht(e,n);Pt(t);const s=t.querySelector(".toc-foldall"),r=n.filter(Ut);if(s&&r.length){const l=(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?{collapse:"全部折叠",expand:"全部展开"}:{collapse:"Collapse all",expand:"Expand all"},a=()=>{const u=r.some(x=>!x.classList.contains("collapsed"));s.innerHTML=u?Wt:Bt,s.setAttribute("aria-label",u?l.collapse:l.expand),s.setAttribute("title",u?l.collapse:l.expand)};s.addEventListener("click",()=>{const u=r.some(x=>!x.classList.contains("collapsed"));_t(e,n,u),a()}),a()}else s&&s.remove();n.forEach((d,l)=>{const a=d.querySelector(".toc-item-text");a&&a.addEventListener("click",()=>{const u=e[l];!u||!u.element||(u.virtual&&u.element.tagName==="DETAILS"&&!u.element.open&&(u.element.open=!0),u.element.scrollIntoView({behavior:"smooth",block:u.virtual?"start":"center"}),setTimeout(()=>i.refresh(),300))})})}function Te(){new Ue,new it,new ot,document.getElementById("map")&&new at,new rt,setTimeout(()=>{new lt},500),new ct,new dt,new pt,St(),Gt(),st(),document.getElementById("tag-graph")&&Lt()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Te):Te();
