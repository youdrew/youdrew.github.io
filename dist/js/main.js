...
`,Xe="(min-width: 1099px)",Ke="(prefers-reduced-motion: reduce)",Ze=8e3,Qe=30,Je=3.4,et=1.3,we=.55,xe=1100,Ee=1,tt=3,nt=1.7;function it(){return/(^|\s)path-(zh-CN-)?index-html(\s|$)/.test(document.body.className||"")}function V(a){return Math.max(0,Math.min(1,a))}function se(a){return a*a*(3-2*a)}const st=`#version 300 es
in vec2 p;
...
void main() {
  ...
  _stColor = c;
    ...
    `,i.querySelector(".canvas-modal__viewport").appendChild(n),document.body.appendChild(i),document.body.classList.add("canvas-modal-open");const s=new Pt(n),r=c=>{c.key==="Escape"&&l()},l=()=>{s.destroy(),i.remove(),document.body.classList.remove("canvas-modal-open"),document.removeEventListener("keydown",r)};i.querySelector(".canvas-modal__close").addEventListener("click",l),i.querySelector(".canvas-modal__overlay").addEventListener("click",l),document.addEventListener("keydown",r),i.querySelectorAll(".canvas-modal__btn").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.action;o==="zoom-in"?s.zoomBy(Ce):o==="zoom-out"?s.zoomBy(1/Ce):o==="reset"&&s.reset()})})}}function It(a){const e=a.querySelector("#canvas-arrow");if(!e)return;Me+=1;const t=`${At}${Me}`;e.id=t,a.querySelectorAll("[marker-end]").forEach(n=>{n.setAttribute("marker-end",`url(#${t})`)})}class Pt{constructor(e){this.svg=e;const t=e.viewBox.baseVal;this.original={x:t.x,y:t.y,w:t.width,h:t.height},this.state={...this.original},this.pointers=new Map,this.pinch=null,this.svg.style.cursor="grab",this.svg.style.touchAction="none",this.onWheel=this.onWheel.bind(this),this.onPointerDown=this.onPointerDown.bind(this),this.onPointerMove=this.onPointerMove.bind(this),this.onPointerUp=this.onPointerUp.bind(this),this.svg.addEventListener("wheel",this.onWheel,{passive:!1}),this.svg.addEventListener("pointerdown",this.onPointerDown),this.svg.addEventListener("pointermove",this.onPointerMove),this.svg.addEventListener("pointerup",this.onPointerUp),this.svg.addEventListener("pointercancel",this.onPointerUp)}setViewBox(){const{x:e,y:t,w:n,h:i}=this.state;this.svg.setAttribute("viewBox",`${e} ${t} ${n} ${i}`)}currentScale(){return this.original.w/this.state.w}zoomBy(e,t,n){const i=this.currentScale()*e;i<kt||i>Mt||(t==null&&(t=this.state.x+this.state.w/2),n==null&&(n=this.state.y+this.state.h/2),this.state.x=t-(t-this.state.x)/e,this.state.y=n-(n-this.state.y)/e,this.state.w/=e,this.state.h/=e,this.setViewBox())}pan(e,t){this.state.x-=e,this.state.y-=t,this.setViewBox()}reset(){this.state={...this.original},this.setViewBox()}screenToSvg(e,t){const n=this.svg.createSVGPoint();n.x=e,n.y=t;const i=this.svg.getScreenCTM();return i?n.matrixTransform(i.inverse()):{x:0,y:0}}onWheel(e){e.preventDefault();const t=e.deltaY<0?ke:1/ke,{x:n,y:i}=this.screenToSvg(e.clientX,e.clientY);this.zoomBy(t,n,i)}onPointerDown(e){e.target.closest("a")||(this.svg.setPointerCapture(e.pointerId),this.pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY,svg:this.screenToSvg(e.clientX,e.clientY)}),this.pointers.size===2?this.pinch=this.computePinch():this.pointers.size===1&&(this.svg.style.cursor="grabbing"))}onPointerMove(e){const t=this.pointers.get(e.pointerId);if(t){if(t.clientX=e.clientX,t.clientY=e.clientY,this.pointers.size===2&&this.pinch){const n=this.computePinch(),i=n.dist/this.pinch.dist;if(i>0&&Number.isFinite(i)){const s=this.screenToSvg(n.cx,n.cy);this.zoomBy(i,s.x,s.y)}this.pinch=n}else if(this.pointers.size===1){const n=this.screenToSvg(e.clientX,e.clientY);this.pan(n.x-t.svg.x,n.y-t.svg.y)}}}onPointerUp(e){this.pointers.delete(e.pointerId),this.pointers.size<2&&(this.pinch=null),this.pointers.size===0&&(this.svg.style.cursor="grab")}computePinch(){const[e,t]=[...this.pointers.values()],n=t.clientX-e.clientX,i=t.clientY-e.clientY;return{dist:Math.hypot(n,i),cx:(e.clientX+t.clientX)/2,cy:(e.clientY+t.clientY)/2}}destroy(){this.svg.removeEventListener("wheel",this.onWheel),this.svg.removeEventListener("pointerdown",this.onPointerDown),this.svg.removeEventListener("pointermove",this.onPointerMove),this.svg.removeEventListener("pointerup",this.onPointerUp),this.svg.removeEventListener("pointercancel",this.onPointerUp)}}const ue={en:{Home:"Home",Daily:"Daily",Archives:"Archives",About:"About",Portfolio:"Portfolio",Bilibili:"Bilibili",GitHub:"GitHub",Instagram:"Instagram",Douban:"Douban",Email:"Email",RSS:"RSS",Language:"Language",Copyright:"Copyright","Powered by":"Powered by","Modified based on":"Modified based on",theme:"theme","Mainly maintained using AI":"Mainly maintained using AI","Older Posts":"Older Posts","Newer Posts":"Newer Posts",Comments:"Comments","Switch to Chinese":"Switch to Chinese","Switch to English":"Switch to English","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"Switched to English",articleLanguageSwitched:"Switched to English version","tag-graph-hint":"Click tag to explore · Scroll to zoom · Drag to rearrange"},"zh-CN":{Home:"首页",Daily:"资讯",Archives:"归档",About:"关于",Portfolio:"作品集",Bilibili:"哔哩哔哩",GitHub:"GitHub",Instagram:"Instagram",Douban:"豆瓣",Email:"邮箱",RSS:"RSS",Language:"语言",Copyright:"版权所有","Powered by":"技术支持","Modified based on":"基于",theme:"主题","Mainly maintained using AI":"主要使用AI来维护","Older Posts":"上一页","Newer Posts":"下一页",Comments:"留言","Switch to Chinese":"切换至中文","Switch to English":"切换至英文","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"已切换至中文",articleLanguageSwitched:"已切换至中文版本","tag-graph-hint":"点击标签探索 · 滚轮缩放 · 拖拽移动"}},Rt=()=>(navigator.language||navigator.userLanguage).startsWith("zh")?"zh-CN":"en",Ne=()=>localStorage.getItem("siteLanguage")||Rt(),Nt=()=>{const a=document.cookie.match(/(?:^|;\s*)lang_pref=([^;]+)/);return a?a[1]:null},zt=a=>{document.cookie="lang_pref="+a+"; path=/; max-age=31536000; samesite=lax"},ze=()=>{const a=document.querySelector('meta[name="article:lang"]');return a?a.content:window.location.pathname.includes(".zh-CN")?"zh-CN":"en"},_e=a=>{const e=document.querySelector(`link[rel="alternate"][hreflang="${a}"]`);if(!e)return null;const t=new URL(e.href,window.location.origin);return window.location.origin+t.pathname+t.search+t.hash},De=a=>{const e=window.location.pathname;if(a==="zh-CN")return e.startsWith("/zh-CN/")||e==="/zh-CN"?null:window.location.origin+"/zh-CN"+e+window.location.search+window.location.hash;if(!e.startsWith("/zh-CN/")&&e!=="/zh-CN")return null;const t=e.replace(/^\/zh-CN(?=\/|$)/,"")||"/";return window.location.origin+t+window.location.search+window.location.hash},qe=a=>{document.querySelectorAll(".lang-switch__opt").forEach(e=>{e.setAttribute("aria-pressed",e.getAttribute("data-lang")===a?"true":"false")})},fe=a=>{const e=ue[a];if(!e){console.warn("Language data not available for:",a);return}document.documentElement.lang=a,document.querySelectorAll("nav ul li a").forEach(c=>{const o=c.getAttribute("data-i18n-key");o&&e[o]&&(c.textContent=e[o])}),document.querySelectorAll("[data-i18n]").forEach(c=>{const o=c.getAttribute("data-i18n");e[o]&&(c.textContent=e[o])}),document.querySelectorAll("[data-title]").forEach(c=>{const o=c.getAttribute("data-title");e[o]&&c.setAttribute("data-title",e[o])});const s=document.querySelector(".pagination .extend.prev"),r=document.querySelector(".pagination .extend.next");s&&(s.textContent=e["Older Posts"]||s.textContent),r&&(r.textContent=e["Newer Posts"]||r.textContent),localStorage.setItem("siteLanguage",a),document.querySelectorAll("[data-i18n-tag]").forEach(c=>{const o=c.getAttribute("data-i18n-tag");if(a==="zh-CN"){const u=window.tagTranslations&&window.tagTranslations[o];u&&(c.textContent=u)}else c.textContent=o}),qe(a)},_t=a=>{const e=document.querySelector(".lang-notification");e&&e.remove();const t=document.createElement("div");t.className="lang-notification",t.textContent=a,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.parentNode&&t.parentNode.removeChild(t)},300)},2e3)},Fe=a=>{if(zt(a),a===ze()){localStorage.setItem("siteLanguage",a),fe(a);return}const e=_e(a)||De(a);if(e){localStorage.setItem("siteLanguage",a),window.location.href=e;return}fe(a);const t=ue[a]?ue[a].languageSwitched:"Language switched";_t(t)},Dt=()=>{const a=Ne()==="zh-CN"?"en":"zh-CN";Fe(a)},qt=()=>{document.querySelectorAll(".lang-switch__opt").forEach(a=>{a.addEventListener("click",e=>{e.preventDefault(),Fe(a.getAttribute("data-lang"))})}),qe(Ne())},Ae=()=>{const a=ze(),e=Nt();if(fe(e||a),e&&e!==a){const t=_e(e)||De(e);t&&window.location.replace(t)}};function Ft(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ae):Ae(),window.addEventListener("load",()=>{const a=document.getElementById("langSwitch");a&&a.addEventListener("click",e=>{e.preventDefault(),Dt()}),qt()})}function Ot(){const a=document.getElementById("tag-graph"),e=document.getElementById("tag-graph-container"),t=window.__TAG_GRAPH_DATA__;if(!a||!t||!t.nodes||t.nodes.length===0)return;const n=document.createElement("div");n.className="graph-loading",n.textContent="Loading",a.parentNode.appendChild(n);const i=t.archiveFilterTags||[],s={};i.forEach(function(d){s[d]=!0});function r(d){return String(d||"").replace(/-/g," ").replace(/\s+/g," ").trim().toLowerCase()}const l={};t.links.forEach(function(d){l[d.source]=(l[d.source]||0)+1,l[d.target]=(l[d.target]||0)+1});let c=1;t.nodes.forEach(function(d){const h=d.value||0;h>c&&(c=h)}),t.nodes.forEach(function(d){const h=d.value||0;d.symbolSize=Math.max(12,Math.min(70,12+h*(58/c)))});const o={};t.nodes.forEach(function(d){o[d.name]=[]});function u(d,h){!o[d]||!o[h]||d===h||(o[d].indexOf(h)===-1&&o[d].push(h),o[h].indexOf(d)===-1&&o[h].push(d))}t.links.forEach(function(d){u(d.source,d.target)});const g={};t.nodes.forEach(function(d){const h=r(d.name);g[h]||(g[h]=[]),g[h].push(d.name)}),Object.keys(g).forEach(function(d){const h=g[d];if(!(h.length<2))for(let v=0;v<h.length;v++)for(let E=v+1;E<h.length;E++)u(h[v],h[E])});const f={},w=[];i.forEach(function(d){o[d]!==void 0&&(f[d]=0,w.push(d))});let b=0;for(;b<w.length;){const d=w[b++];(o[d]||[]).forEach(function(h){f[h]===void 0&&(f[h]=f[d]+1,w.push(h))})}Object.keys(f).forEach(function(d){});const C=[{h:260,s:62,l:50},{h:15,s:80,l:55},{h:160,s:60,l:42},{h:220,s:72,l:52},{h:340,s:70,l:52},{h:45,s:85,l:50},{h:190,s:70,l:45},{h:90,s:55,l:45},{h:290,s:60,l:50},{h:30,s:75,l:48},{h:130,s:50,l:42},{h:0,s:70,l:55}];function Y(d,h,v){return"hsl("+Math.round(d)+", "+Math.round(h)+"%, "+Math.round(v)+"%)"}const $={},oe={},D=t.nodes.filter(function(d){return s[d.name]});D.sort(function(d,h){return h.value-d.value}),D.forEach(function(d,h){const v=C[h%C.length];oe[d.name]=v,$[d.name]=Y(v.h,v.s,v.l)});const H={};i.forEach(function(d){if(o[d]===void 0)return;const h={};h[d]=0;const v=[d];let E=0;for(;E<v.length;){const P=v[E++];(o[P]||[]).forEach(function(M){h[M]===void 0&&(h[M]=h[P]+1,v.push(M))})}H[d]=h});const Q=t.nodes.filter(function(d){return!s[d.name]});let re=1;Q.forEach(function(d){let h=1/0;i.forEach(function(v){if(!H[v])return;const E=H[v][d.name];E!==void 0&&E<h&&(h=E)}),h<1/0&&h>re&&(re=h)}),Q.forEach(function(d){const h=[];let v=0;if(i.forEach(function(p){if(!H[p]||!oe[p])return;let y=H[p][d.name];if(y===void 0)return;y===0&&(y=.5);const L=1/(y*y);h.push({ft:p,w:L}),v+=L}),v===0){$[d.name]="hsl(0, 0%, 82%)";return}let E=0,P=0,M=0,x=0;h.forEach(function(p){const y=p.w/v,L=oe[p.ft],N=L.h*Math.PI/180;E+=Math.sin(N)*y,P+=Math.cos(N)*y,M+=L.s*y,x+=L.l*y});let A=Math.atan2(E,P)*180/Math.PI;A<0&&(A+=360);let z=M,_=x,m=1/0;h.forEach(function(p){const y=H[p.ft][d.name];y<m&&(m=y)});let S=(m-1)/Math.max(re-1,1);S=Math.max(0,Math.min(1,S));const I=Math.pow(S,.85),T=32,k=1-I*.35;z=Math.max(T,z*k),_=_+I*(82-_)*.78,$[d.name]=Y(A,z,_)});const le=t.tagTranslations||{},J={};Object.keys(le).forEach(function(d){J[d]=le[d];const h=d.replace(/-/g," ");h!==d&&(J[h]=le[d])});function ee(d){return(typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en")==="zh-CN"&&J[d]?J[d]:d}const ve=e.getBoundingClientRect(),F=ve.width||500,O=ve.height||400,ge=Math.min(F,O)*.28;D.forEach(function(d,h){const v=2*Math.PI*h/Math.max(D.length,1)-Math.PI/2;d.x=F/2+ge*Math.cos(v),d.y=O/2+ge*Math.sin(v)});const ye=Math.min(F,O)*.45;Q.forEach(function(d,h){const v=2*Math.PI*h/Math.max(Q.length,1);d.x=F/2+ye*Math.cos(v),d.y=O/2+ye*Math.sin(v)});const We=60,R=t.nodes;for(let d=0;d<15;d++)for(let h=0;h<R.length;h++)for(let v=h+1;v<R.length;v++){const E=R[v].x-R[h].x,P=R[v].y-R[h].y,M=Math.sqrt(E*E+P*P),x=We+(R[h].symbolSize+R[v].symbolSize)/2;if(M<x){const A=(x-M)/2,z=M>.1?E/M:Math.random()-.5,_=M>.1?P/M:Math.random()-.5;R[h].x-=z*A,R[h].y-=_*A,R[v].x+=z*A,R[v].y+=_*A}}let X=1,K=[F/2,O/2];if(D.length>0){let d=1/0,h=-1/0,v=1/0,E=-1/0;D.forEach(function(x){const A=(x.symbolSize||20)/2+50;x.x-A<d&&(d=x.x-A),x.x+A>h&&(h=x.x+A),x.y-A<v&&(v=x.y-A),x.y+A>E&&(E=x.y+A)});const P=h-d,M=E-v;if(P>0&&M>0){const x=F/P,A=O/M;X=Math.min(x,A,1.5)*.8,X<.3&&(X=.3),K=[(d+h)/2,(v+E)/2]}}t.nodes.forEach(function(d){d.itemStyle={color:$[d.name],borderColor:"#fff",borderWidth:1.5,shadowBlur:5,shadowColor:"rgba(0, 0, 0, 0.06)"},d.label={show:!0,formatter:function(){return ee(d.name)},fontSize:Math.max(10,Math.min(15,9+(l[d.name]||0)*.5)),color:"#555",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'}});const B=e.querySelector(".tag-graph-hint");let be=null,ce=!1;function Ue(){ce||(ce=!0,B.classList.add("visible"),clearTimeout(be),be=setTimeout(function(){B.classList.remove("visible"),ce=!1},3e3))}e.addEventListener("mouseenter",Ue);const W=document.createElement("script");W.src="https://cdn.jsdelivr.net/npm/echarts@5.5.1/dist/echarts.min.js",W.integrity="sha384-Mx5lkUEQPM1pOJCwFtUICyX45KNojXbkWdYhkKUKsbv391mavbfoAmONbzkgYPzR",W.crossOrigin="anonymous",W.onload=function(){Ge()},W.onerror=function(){n.textContent="Failed to load chart library",n.style.color="#c44"},document.head.appendChild(W);function je(d){return d<10?750:d<20?1200:d<40?1650:2100}function Ge(){n.parentNode&&n.parentNode.removeChild(n);const d=window.matchMedia&&window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window,h=echarts.init(a),v={backgroundColor:"transparent",tooltip:{show:!0,enterable:!0,confine:!0,backgroundColor:"rgba(255, 255, 255, 0.97)",borderColor:"#e8e8e8",borderWidth:1,padding:[10,14],textStyle:{color:"#4b4848",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',fontSize:13},extraCssText:"border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); max-height: 260px; overflow-y: auto;",formatter:function(m){function S(T){return String(T??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function I(T,k){const p=S(T),y=S(k),L='style="display:block;color:#666;font-size:11px;line-height:1.6;padding:1px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px;text-decoration:none;"';return y?'<a href="'+y+'" '+L+">• "+p+"</a>":"<div "+L+">• "+p+"</div>"}if(m.dataType==="node"){const T=ee(m.name);let k='<div style="font-weight:600;font-size:14px;margin-bottom:5px;color:'+($[m.name]||"#795da3")+'">'+S(T)+"</div>";k+='<div style="color:#777;font-size:12px;margin-bottom:6px;">📄 '+m.value+" article"+(m.value>1?"s":"")+"</div>";const p=t.postTitles&&t.postTitles[m.name];return p&&p.length>0&&(k+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;">',p.forEach(function(y){typeof y=="string"?k+=I(y,""):k+=I(y.title,y.path)}),k+="</div>"),k}if(m.dataType==="edge"){const T=m.data.source,k=m.data.target;let p='<span style="font-weight:600">'+S(ee(T))+'</span> <span style="color:#bbb">↔</span> <span style="font-weight:600">'+S(ee(k))+"</span>";p+='<br/><span style="color:#999;font-size:12px">📄 '+m.data.value+" article"+(m.data.value>1?"s":"")+"</span>";const y=[T,k].sort().join("	"),L=t.linkPosts&&t.linkPosts[y];return L&&L.length>0&&(p+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;margin-top:5px;">',L.forEach(function(N){p+=I(N.title,N.path)}),p+="</div>"),p}}},animationDuration:1500,animationEasingUpdate:"quinticInOut",series:[{type:"graph",layout:"force",data:t.nodes,links:t.links,roam:!1,draggable:!d,force:{repulsion:je(t.nodes.length),edgeLength:[150,450],gravity:.12,friction:.6,layoutAnimation:!0},emphasis:{focus:"adjacency",blurScope:"global",itemStyle:{shadowBlur:20,shadowColor:"rgba(121, 93, 163, 0.45)",borderWidth:2,borderColor:"#fff"},lineStyle:{width:3,opacity:.85},label:{show:!0,fontSize:14,fontWeight:"bold",color:"#333"}},label:{position:"right",distance:6},lineStyle:{color:"#d0d0d0",width:1.5,curveness:0,opacity:.35},scaleLimit:{min:.3,max:4},zoom:X,center:K}]};h.setOption(v);let E=!1,P=!1;if(D.length>0){const m=function(){if(P)return;if(E){P=!0,h.off("finished",m);return}const S=h.getModel(),I=S&&S.getSeriesByIndex&&S.getSeriesByIndex(0),T=I&&I.getGraph&&I.getGraph();let k=1/0,p=-1/0,y=1/0,L=-1/0,N=0;if(D.forEach(function(ne){const de=T&&T.getNodeByName&&T.getNodeByName(ne.name),ie=de&&de.getLayout&&de.getLayout();let j,G;ie&&ie.length>=2?(j=ie[0],G=ie[1]):(j=ne.x||0,G=ne.y||0);const q=(ne.symbolSize||20)/2+50;j-q<k&&(k=j-q),j+q>p&&(p=j+q),G-q<y&&(y=G-q),G+q>L&&(L=G+q),N++}),N===0)return;const Z=p-k,te=L-y;if(Z<=0||te<=0)return;let U=Math.min(F/Z,O/te,1.5)*.8;U<.3&&(U=.3),P=!0,M=U,x=[(k+p)/2,(y+L)/2],h.setOption({series:[{zoom:U,center:x.slice()}]}),h.off("finished",m)};h.on("finished",m)}h.on("click",function(m){m.dataType==="node"&&t.tagPaths&&t.tagPaths[m.name]&&(window.location.href=t.tagPaths[m.name])}),h.on("mouseover",function(m){(m.dataType==="node"||m.dataType==="edge")&&(a.style.cursor="pointer")}),h.on("mouseout",function(){a.style.cursor="default"});let M=X||1,x=K?[K[0],K[1]]:[0,0];if(!d){const m=e||a;m.addEventListener("wheel",function(p){p.preventDefault()},{passive:!1}),m.addEventListener("touchmove",function(p){p.touches.length>=2&&p.preventDefault()},{passive:!1});const S=h.getZr();S.on("mousewheel",function(p){p.event.preventDefault(),p.event.stopPropagation(),E=!0;const y=p.wheelDelta>0?1.08:1/1.08;let L=M*y;L<.3&&(L=.3),L>4&&(L=4),M=L,h.setOption({series:[{zoom:M}]})});let I=!1,T=[0,0],k=[0,0];S.on("mousedown",function(p){p.target||(I=!0,E=!0,T=[p.event.clientX,p.event.clientY],k=[x[0],x[1]],a.style.cursor="grabbing")}),S.on("mousemove",function(p){if(I){const y=p.event.clientX-T[0],L=p.event.clientY-T[1],N=a.clientWidth,Z=a.clientHeight,te=N/M,U=Z/M;x[0]=k[0]-y*(te/N),x[1]=k[1]-L*(U/Z),h.setOption({series:[{center:[x[0],x[1]]}]})}}),S.on("mouseup",function(){I&&(I=!1,a.style.cursor="default")}),S.on("globalout",function(){I&&(I=!1,a.style.cursor="default")})}if(d){const m=document.createElement("button");m.type="button",m.className="tag-graph-fs-btn",m.setAttribute("aria-label","Fullscreen"),m.innerHTML='<svg class="tag-graph-fs-ic tag-graph-fs-ic--open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg><svg class="tag-graph-fs-ic tag-graph-fs-ic--close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',e.appendChild(m);let S=!1;const I=function(){if(!B)return;const p=typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en";B.textContent=p==="zh-CN"?"双指缩放 · 拖动平移 · 点按进入标签":"Pinch to zoom · Drag to pan · Tap a tag",B.classList.add("visible"),setTimeout(function(){B.classList.remove("visible")},2600)},T=function(){S=!0,e.classList.add("tag-graph-fullscreen"),m.classList.add("is-fullscreen"),m.setAttribute("aria-label","Exit fullscreen"),document.body.style.overflow="hidden",document.body.classList.add("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!0}]}),I()})},k=function(){S=!1,e.classList.remove("tag-graph-fullscreen"),m.classList.remove("is-fullscreen"),m.setAttribute("aria-label","Fullscreen"),document.body.style.overflow="",document.body.classList.remove("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!1,zoom:M,center:x.slice()}]})})};m.addEventListener("click",function(p){p.preventDefault(),p.stopPropagation(),S?k():T()}),document.addEventListener("keydown",function(p){p.key==="Escape"&&S&&k()})}let A;window.addEventListener("resize",function(){clearTimeout(A),A=setTimeout(function(){h.resize()},150)});function z(){h.setOption({series:[{data:t.nodes}]})}window.addEventListener("storage",function(m){m.key==="siteLanguage"&&z()});const _=localStorage.setItem;localStorage.setItem=function(m,S){_.call(localStorage,m,S),m==="siteLanguage"&&setTimeout(z,50)}}}function Ht(a){if(a.querySelector("audio"))return!1;const e=a.querySelector("summary");return!(!e||/跟读|本节语音/.test(e.textContent||"")||a.parentElement&&a.parentElement.closest("details.callout"))}function Bt(a,e={}){const t=e.includeCallouts?"h1, h2, h3, h4, h5, h6, details.callout--foldable":"h1, h2, h3, h4, h5, h6",n=Array.from(a.querySelectorAll(t)),i=[];let s=1;const r=[];return n.forEach(l=>{const c=/^H[1-6]$/.test(l.tagName);let o,u,g=!1;if(c)o=parseInt(l.tagName[1],10),s=o,u=l.textContent;else{if(!Ht(l))return;o=s+1,g=!0;const C=l.querySelector("summary");if(u=Pe(C.textContent||"",l.getAttribute("data-callout")||"",{short:!0}),!u)return}let f=1;for(;i.length&&i[i.length-1].level>=o;){const C=i.pop();C.level===o&&(f=C.n+1)}i.push({level:o,n:f});const w=r.length;l.id||(l.id=g?`toc-item-${w}`:`heading-${w}`);const b=i.map(C=>C.n).join(".");l.dataset.tocNumber=b,r.push({element:l,level:o,index:w,id:l.id,text:u,number:b,virtual:g})}),r}function Wt(a){const e=document.createElement("aside");e.className="toc-drawer",e.innerHTML='<div class="toc-header"><span class="toc-title"></span><div class="toc-header__actions"><button type="button" class="toc-foldall"></button><button type="button" class="toc-close-btn"><span class="toc-close-btn__x" aria-hidden="true"></span></button></div></div><div class="toc-content"><div class="toc-list"></div></div>',document.body.appendChild(e);const t=e.querySelector(".toc-list"),n=a.map(i=>{const s=document.createElement("div");if(s.className=i.virtual?"toc-item toc-item--virtual":"toc-item",s.setAttribute("data-level",String(i.level)),s.setAttribute("data-index",String(i.index)),!i.virtual){const l=document.createElement("div");l.className="toc-collapse-btn",s.appendChild(l)}const r=document.createElement("span");return r.className="toc-item-text",r.style.cursor="pointer",r.innerHTML=`<span class="toc-number">${i.number}.</span> `,r.appendChild(document.createTextNode(i.text)),r.setAttribute("title",i.text),i.element.classList.contains("collapsed")&&s.classList.add("collapsed"),s.appendChild(r),t.appendChild(s),s});return{container:e,items:n}}const Oe="toc-panel-state";function He(){try{const a=localStorage.getItem(Oe);if(!a)return null;const e=JSON.parse(a);return!e||typeof e!="object"?null:e}catch{return null}}function Ut(a){try{const t={...He()||{},...a};localStorage.setItem(Oe,JSON.stringify(t))}catch{}}function jt(){return(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?"zh":"en"}const Gt={zh:{title:"目录",open:"目录",hide:"收起目录",show:"打开目录"},en:{title:"Contents",open:"TOC",hide:"Hide contents",show:"Show contents"}};function Vt(a){const e=a.querySelector(".toc-content"),t=a.querySelector(".toc-item.toc-reading");!e||!t||(e.scrollTop=t.offsetTop-e.clientHeight/2+t.offsetHeight/2)}function Yt(a){const e=Gt[jt()],t=a.querySelector(".toc-title");t&&(t.textContent=e.title),a.setAttribute("aria-label",e.title);const n=a.querySelector(".toc-close-btn");n&&(n.setAttribute("aria-label",e.hide),n.setAttribute("title",e.hide));const i=document.createElement("button");i.type="button",i.className="toc-tab",i.setAttribute("aria-label",e.show),i.innerHTML='<span class="toc-tab__icon" aria-hidden="true"></span><span class="toc-tab__text">'+e.open+"</span>",document.body.appendChild(i);const s=document.createElement("div");s.className="toc-scrim",document.body.appendChild(s);function r(o,u){a.classList.toggle("is-open",o),i.classList.toggle("is-hidden",o),s.classList.toggle("is-visible",o),document.body.classList.toggle("toc-drawer-open",o),o&&Vt(a),u&&Ut({hidden:!o})}i.addEventListener("click",()=>r(!0,!0)),s.addEventListener("click",()=>r(!1,!0)),n&&n.addEventListener("click",o=>{o.stopPropagation(),r(!1,!0)}),document.addEventListener("keydown",o=>{o.key==="Escape"&&a.classList.contains("is-open")&&r(!1,!1)});const l=He(),c=window.matchMedia("(min-width: 1100px)").matches;return r(c&&!!l&&l.hidden===!1,!1),{setOpen:r}}function me(a,e,t){for(let n=e+1;n<a.length&&!(parseInt(a[n].getAttribute("data-level")||"1",10)<=t);n+=1)a[n].classList.add("toc-hidden")}function Be(a,e,t){for(let n=e+1;n<a.length;n+=1){const i=parseInt(a[n].getAttribute("data-level")||"1",10);if(i<=t)break;if(i===t+1)a[n].classList.remove("toc-hidden");else{let s=!0;for(let r=n-1;r>e;r-=1){const l=parseInt(a[r].getAttribute("data-level")||"1",10);if(l<i&&a[r].classList.contains("collapsed")){s=!1;break}if(l<=t)break}s&&a[n].classList.remove("toc-hidden")}}}function $t(a){const e=parseInt(a.tagName.charAt(1),10);let t=a.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t=t.nextElementSibling;continue}t.style.display="none",t=t.nextElementSibling}}function Xt(a){const e=parseInt(a.tagName.charAt(1),10);let t=a.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t.style.display="",t=t.nextElementSibling;continue}t.style.display="",t=t.nextElementSibling}}function pe(a,e,t){const n=e[t],i=a[t]&&a[t].element;if(!n||!i)return;const s=parseInt(n.getAttribute("data-level")||"1",10);!n.classList.contains("collapsed")?(n.classList.add("collapsed"),me(e,t,s),i.classList.contains("collapsed")||(i.classList.add("collapsed"),$t(i))):(n.classList.remove("collapsed"),Be(e,t,s),i.classList.contains("collapsed")&&(i.classList.remove("collapsed"),Xt(i)))}function Kt(a,e,t){e.forEach((n,i)=>{if(n.classList.contains("toc-item--virtual")||!n.querySelector(".toc-collapse-btn"))return;const s=n.classList.contains("collapsed");t?!s&&!n.classList.contains("toc-hidden")&&pe(a,e,i):s&&pe(a,e,i)})}function Zt(a,e,t){const n=e[t],i=a[t]&&a[t].element;if(!n||!i)return;const s=parseInt(n.getAttribute("data-level")||"1",10),r=i.classList.contains("collapsed");r&&!n.classList.contains("collapsed")?(n.classList.add("collapsed"),me(e,t,s)):!r&&n.classList.contains("collapsed")&&(n.classList.remove("collapsed"),Be(e,t,s))}function Qt(a,e){e.forEach((i,s)=>{if(i.classList.contains("collapsed")){const r=parseInt(i.getAttribute("data-level")||"1",10);me(e,s,r)}}),e.forEach((i,s)=>{const r=i.querySelector(".toc-collapse-btn");r&&r.addEventListener("click",l=>{l.stopPropagation(),pe(a,e,s)})});const t=new Map;a.forEach((i,s)=>t.set(i.element,s));const n=new MutationObserver(i=>{i.forEach(s=>{if(s.type!=="attributes"||s.attributeName!=="class")return;const r=t.get(s.target);r!==void 0&&Zt(a,e,r)})});return a.forEach(i=>{n.observe(i.element,{attributes:!0,attributeFilter:["class"]})}),{observer:n}}const Te={1:{passed:"rgba(128,128,128,0.1)",reading:"rgba(66,153,225,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(66,153,225,0.25)"},2:{passed:"rgba(128,128,128,0.1)",reading:"rgba(49,130,206,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(49,130,206,0.25)"},3:{passed:"rgba(128,128,128,0.1)",reading:"rgba(44,82,130,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(44,82,130,0.25)"},4:{passed:"rgba(128,128,128,0.1)",reading:"rgba(42,67,101,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(42,67,101,0.25)"},5:{passed:"rgba(128,128,128,0.1)",reading:"rgba(26,54,93,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(26,54,93,0.25)"},6:{passed:"rgba(128,128,128,0.1)",reading:"rgba(21,62,117,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(21,62,117,0.25)"}};function Jt(a,e){if(!a.length)return{destroy(){}};const t=new Array(a.length).fill("coming"),n=new Set;let i=-1;const s=new Map;a.forEach((f,w)=>s.set(f.element,w));function r(){e.forEach((f,w)=>{const b=parseInt(f.getAttribute("data-level")||"1",10),C=Te[b]||Te[1],Y=t[w];f.classList.remove("toc-passed","toc-reading","toc-coming"),f.style.boxShadow="",f.style.transform="",f.style.fontWeight="",w===i?(f.classList.add("toc-reading"),f.style.backgroundColor=C.active,f.style.opacity="1",f.style.fontWeight="600",f.style.boxShadow="inset 0 0 0 2px rgba(66,153,225,0.3)",f.style.transform="scale(1.02)",f.style.transition="all 0.2s ease"):Y==="reading"?(f.classList.add("toc-reading"),f.style.backgroundColor=C.reading,f.style.opacity="1",f.style.fontWeight="600"):Y==="passed"?(f.classList.add("toc-passed"),f.style.backgroundColor=C.passed,f.style.opacity="0.7"):(f.classList.add("toc-coming"),f.style.backgroundColor=C.coming,f.style.opacity="0.5")})}function l(){const f=window.innerHeight/2;let w=-1;n.forEach(b=>{const C=a[b].element.getBoundingClientRect();C.top<=f&&C.bottom>=f&&(w=b)}),w!==i&&(i=w,r())}let c=null;function o(){c||(c=requestAnimationFrame(()=>{c=null,l()}))}const u=new IntersectionObserver(f=>{f.forEach(w=>{const b=s.get(w.target);b!==void 0&&(w.isIntersecting?(n.add(b),t[b]="reading"):(n.delete(b),t[b]=w.boundingClientRect.bottom<0?"passed":"coming"))}),l(),r()});a.forEach(f=>u.observe(f.element)),window.addEventListener("scroll",o,{passive:!0}),window.addEventListener("resize",o,{passive:!0}),r();function g(){u.disconnect(),window.removeEventListener("scroll",o),window.removeEventListener("resize",o),c&&cancelAnimationFrame(c)}return{destroy:g,refresh:()=>{l(),r()}}}const en='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M7.41 18.59 8.83 20 12 16.83 15.17 20l1.41-1.41L12 14zM16.59 5.41 15.17 4 12 7.17 8.83 4 7.41 5.41 12 10z"/></svg>',tn='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 5.83 15.17 9l1.41-1.41L12 3 7.42 7.59 8.83 9zm0 12.34L8.83 15l-1.41 1.4L12 21l4.58-4.6L15.17 15z"/></svg>';function nn(a){return!a.classList.contains("toc-item--virtual")&&!!a.querySelector(".toc-collapse-btn")}function sn(){const a=document.querySelector(".content");if(!a||a.classList.contains("archives"))return null;const e=document.body;if(e.classList.contains("path-about-index-html")||e.classList.contains("layout-about"))return null;const t=document.querySelector("section.main[data-toc]");return t&&t.getAttribute("data-toc")==="false"?null:a}function an(){const a=sn();if(!a)return;const e=Bt(a,{includeCallouts:document.body.classList.contains("type-daily-feed")});if(!e.length)return;const{container:t,items:n}=Wt(e);Qt(e,n);const i=Jt(e,n);Yt(t);const s=t.querySelector(".toc-foldall"),r=n.filter(nn);if(s&&r.length){const c=(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?{collapse:"全部折叠",expand:"全部展开"}:{collapse:"Collapse all",expand:"Expand all"},o=()=>{const u=r.some(g=>!g.classList.contains("collapsed"));s.innerHTML=u?en:tn,s.setAttribute("aria-label",u?c.collapse:c.expand),s.setAttribute("title",u?c.collapse:c.expand)};s.addEventListener("click",()=>{const u=r.some(g=>!g.classList.contains("collapsed"));Kt(e,n,u),o()}),o()}else s&&s.remove();n.forEach((l,c)=>{const o=l.querySelector(".toc-item-text");o&&o.addEventListener("click",()=>{const u=e[c];!u||!u.element||(u.virtual&&u.element.tagName==="DETAILS"&&!u.element.open&&(u.element.open=!0),u.element.scrollIntoView({behavior:"smooth",block:u.virtual?"start":"center"}),setTimeout(()=>i.refresh(),300))})})}function Ie(){new Ve,new ot,new wt,document.getElementById("map")&&new xt,new Et,setTimeout(()=>{new St},500),new Lt,new Ct,new Tt,Ft(),an(),new bt,document.getElementById("tag-graph")&&Ot()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ie):Ie();
