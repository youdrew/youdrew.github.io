...
`,Xe="(min-width: 1099px)",Ke="(prefers-reduced-motion: reduce)",Ze=8e3,Qe=30,Je=3.4,et=1.3,we=.55,xe=1100,Ee=1,tt=3,nt=1.7;function it(){return/(^|\s)path-(zh-CN-)?index-html(\s|$)/.test(document.body.className||"")}function X(s){return Math.max(0,Math.min(1,s))}function ae(s){return s*s*(3-2*s)}const st=`#version 300 es
in vec2 p;
...
void main() {
  ...
  _stColor = c;
    ...
    `,i.querySelector(".canvas-modal__viewport").appendChild(n),document.body.appendChild(i),document.body.classList.add("canvas-modal-open");const a=new Nt(n),r=c=>{c.key==="Escape"&&l()},l=()=>{a.destroy(),i.remove(),document.body.classList.remove("canvas-modal-open"),document.removeEventListener("keydown",r)};i.querySelector(".canvas-modal__close").addEventListener("click",l),i.querySelector(".canvas-modal__overlay").addEventListener("click",l),document.addEventListener("keydown",r),i.querySelectorAll(".canvas-modal__btn").forEach(c=>{c.addEventListener("click",()=>{const o=c.dataset.action;o==="zoom-in"?a.zoomBy(Ce):o==="zoom-out"?a.zoomBy(1/Ce):o==="reset"&&a.reset()})})}}function Pt(s){const e=s.querySelector("#canvas-arrow");if(!e)return;Me+=1;const t=`${Tt}${Me}`;e.id=t,s.querySelectorAll("[marker-end]").forEach(n=>{n.setAttribute("marker-end",`url(#${t})`)})}class Nt{constructor(e){this.svg=e;const t=e.viewBox.baseVal;this.original={x:t.x,y:t.y,w:t.width,h:t.height},this.state={...this.original},this.pointers=new Map,this.pinch=null,this.svg.style.cursor="grab",this.svg.style.touchAction="none",this.onWheel=this.onWheel.bind(this),this.onPointerDown=this.onPointerDown.bind(this),this.onPointerMove=this.onPointerMove.bind(this),this.onPointerUp=this.onPointerUp.bind(this),this.svg.addEventListener("wheel",this.onWheel,{passive:!1}),this.svg.addEventListener("pointerdown",this.onPointerDown),this.svg.addEventListener("pointermove",this.onPointerMove),this.svg.addEventListener("pointerup",this.onPointerUp),this.svg.addEventListener("pointercancel",this.onPointerUp)}setViewBox(){const{x:e,y:t,w:n,h:i}=this.state;this.svg.setAttribute("viewBox",`${e} ${t} ${n} ${i}`)}currentScale(){return this.original.w/this.state.w}zoomBy(e,t,n){const i=this.currentScale()*e;i<Mt||i>At||(t==null&&(t=this.state.x+this.state.w/2),n==null&&(n=this.state.y+this.state.h/2),this.state.x=t-(t-this.state.x)/e,this.state.y=n-(n-this.state.y)/e,this.state.w/=e,this.state.h/=e,this.setViewBox())}pan(e,t){this.state.x-=e,this.state.y-=t,this.setViewBox()}reset(){this.state={...this.original},this.setViewBox()}screenToSvg(e,t){const n=this.svg.createSVGPoint();n.x=e,n.y=t;const i=this.svg.getScreenCTM();return i?n.matrixTransform(i.inverse()):{x:0,y:0}}onWheel(e){e.preventDefault();const t=e.deltaY<0?ke:1/ke,{x:n,y:i}=this.screenToSvg(e.clientX,e.clientY);this.zoomBy(t,n,i)}onPointerDown(e){e.target.closest("a")||(this.svg.setPointerCapture(e.pointerId),this.pointers.set(e.pointerId,{clientX:e.clientX,clientY:e.clientY,svg:this.screenToSvg(e.clientX,e.clientY)}),this.pointers.size===2?this.pinch=this.computePinch():this.pointers.size===1&&(this.svg.style.cursor="grabbing"))}onPointerMove(e){const t=this.pointers.get(e.pointerId);if(t){if(t.clientX=e.clientX,t.clientY=e.clientY,this.pointers.size===2&&this.pinch){const n=this.computePinch(),i=n.dist/this.pinch.dist;if(i>0&&Number.isFinite(i)){const a=this.screenToSvg(n.cx,n.cy);this.zoomBy(i,a.x,a.y)}this.pinch=n}else if(this.pointers.size===1){const n=this.screenToSvg(e.clientX,e.clientY);this.pan(n.x-t.svg.x,n.y-t.svg.y)}}}onPointerUp(e){this.pointers.delete(e.pointerId),this.pointers.size<2&&(this.pinch=null),this.pointers.size===0&&(this.svg.style.cursor="grab")}computePinch(){const[e,t]=[...this.pointers.values()],n=t.clientX-e.clientX,i=t.clientY-e.clientY;return{dist:Math.hypot(n,i),cx:(e.clientX+t.clientX)/2,cy:(e.clientY+t.clientY)/2}}destroy(){this.svg.removeEventListener("wheel",this.onWheel),this.svg.removeEventListener("pointerdown",this.onPointerDown),this.svg.removeEventListener("pointermove",this.onPointerMove),this.svg.removeEventListener("pointerup",this.onPointerUp),this.svg.removeEventListener("pointercancel",this.onPointerUp)}}const ue={en:{Home:"Home",Daily:"Daily",Archives:"Archives",About:"About",Portfolio:"Portfolio",Bilibili:"Bilibili",GitHub:"GitHub",Instagram:"Instagram",Douban:"Douban",Email:"Email",RSS:"RSS",Language:"Language",Copyright:"Copyright","Powered by":"Powered by","Modified based on":"Modified based on",theme:"theme","Mainly maintained using AI":"Mainly maintained using AI","Older Posts":"Older Posts","Newer Posts":"Newer Posts",Comments:"Comments","Switch to Chinese":"Switch to Chinese","Switch to English":"Switch to English","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"Switched to English",articleLanguageSwitched:"Switched to English version","tag-graph-hint":"Click tag to explore · Scroll to zoom · Drag to rearrange"},"zh-CN":{Home:"首页",Daily:"资讯",Archives:"归档",About:"关于",Portfolio:"作品集",Bilibili:"哔哩哔哩",GitHub:"GitHub",Instagram:"Instagram",Douban:"豆瓣",Email:"邮箱",RSS:"RSS",Language:"语言",Copyright:"版权所有","Powered by":"技术支持","Modified based on":"基于",theme:"主题","Mainly maintained using AI":"主要使用AI来维护","Older Posts":"上一页","Newer Posts":"下一页",Comments:"留言","Switch to Chinese":"切换至中文","Switch to English":"切换至英文","View Chinese Version":"查看中文版","View English Version":"View English Version",languageSwitched:"已切换至中文",articleLanguageSwitched:"已切换至中文版本","tag-graph-hint":"点击标签探索 · 滚轮缩放 · 拖拽移动"}},Rt=()=>(navigator.language||navigator.userLanguage).startsWith("zh")?"zh-CN":"en",Re=()=>localStorage.getItem("siteLanguage")||Rt(),zt=()=>{const s=document.cookie.match(/(?:^|;\s*)lang_pref=([^;]+)/);return s?s[1]:null},_t=s=>{document.cookie="lang_pref="+s+"; path=/; max-age=31536000; samesite=lax"},ze=()=>{const s=document.querySelector('meta[name="article:lang"]');return s?s.content:window.location.pathname.includes(".zh-CN")?"zh-CN":"en"},_e=s=>{const e=document.querySelector(`link[rel="alternate"][hreflang="${s}"]`);if(!e)return null;const t=new URL(e.href,window.location.origin);return window.location.origin+t.pathname+t.search+t.hash},De=s=>{const e=window.location.pathname;if(s==="zh-CN")return e.startsWith("/zh-CN/")||e==="/zh-CN"?null:window.location.origin+"/zh-CN"+e+window.location.search+window.location.hash;if(!e.startsWith("/zh-CN/")&&e!=="/zh-CN")return null;const t=e.replace(/^\/zh-CN(?=\/|$)/,"")||"/";return window.location.origin+t+window.location.search+window.location.hash},qe=s=>{document.querySelectorAll(".lang-switch__opt").forEach(e=>{e.setAttribute("aria-pressed",e.getAttribute("data-lang")===s?"true":"false")})},fe=s=>{const e=ue[s];if(!e){console.warn("Language data not available for:",s);return}document.documentElement.lang=s,document.querySelectorAll("nav ul li a").forEach(c=>{const o=c.getAttribute("data-i18n-key");o&&e[o]&&(c.textContent=e[o])}),document.querySelectorAll("[data-i18n]").forEach(c=>{const o=c.getAttribute("data-i18n");e[o]&&(c.textContent=e[o])}),document.querySelectorAll("[data-title]").forEach(c=>{const o=c.getAttribute("data-title");e[o]&&c.setAttribute("data-title",e[o])});const a=document.querySelector(".pagination .extend.prev"),r=document.querySelector(".pagination .extend.next");a&&(a.textContent=e["Older Posts"]||a.textContent),r&&(r.textContent=e["Newer Posts"]||r.textContent),localStorage.setItem("siteLanguage",s),document.querySelectorAll("[data-i18n-tag]").forEach(c=>{const o=c.getAttribute("data-i18n-tag");if(s==="zh-CN"){const u=window.tagTranslations&&window.tagTranslations[o];u&&(c.textContent=u)}else c.textContent=o}),qe(s)},Dt=s=>{const e=document.querySelector(".lang-notification");e&&e.remove();const t=document.createElement("div");t.className="lang-notification",t.textContent=s,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.parentNode&&t.parentNode.removeChild(t)},300)},2e3)},Fe=s=>{if(_t(s),s===ze()){localStorage.setItem("siteLanguage",s),fe(s);return}const e=_e(s)||De(s);if(e){localStorage.setItem("siteLanguage",s),window.location.href=e;return}fe(s);const t=ue[s]?ue[s].languageSwitched:"Language switched";Dt(t)},qt=()=>{const s=Re()==="zh-CN"?"en":"zh-CN";Fe(s)},Ft=()=>{document.querySelectorAll(".lang-switch__opt").forEach(s=>{s.addEventListener("click",e=>{e.preventDefault(),Fe(s.getAttribute("data-lang"))})}),qe(Re())},Ae=()=>{const s=ze(),e=zt();if(fe(e||s),e&&e!==s){const t=_e(e)||De(e);t&&window.location.replace(t)}};function Ot(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ae):Ae(),window.addEventListener("load",()=>{const s=document.getElementById("langSwitch");s&&s.addEventListener("click",e=>{e.preventDefault(),qt()}),Ft()})}function Ht(){const s=document.getElementById("tag-graph"),e=document.getElementById("tag-graph-container"),t=window.__TAG_GRAPH_DATA__;if(!s||!t||!t.nodes||t.nodes.length===0)return;const n=document.createElement("div");n.className="graph-loading",n.textContent="Loading",s.parentNode.appendChild(n);const i=t.archiveFilterTags||[],a={};i.forEach(function(d){a[d]=!0});function r(d){return String(d||"").replace(/-/g," ").replace(/\s+/g," ").trim().toLowerCase()}const l={};t.links.forEach(function(d){l[d.source]=(l[d.source]||0)+1,l[d.target]=(l[d.target]||0)+1});let c=1;t.nodes.forEach(function(d){const h=d.value||0;h>c&&(c=h)}),t.nodes.forEach(function(d){const h=d.value||0;d.symbolSize=Math.max(12,Math.min(70,12+h*(58/c)))});const o={};t.nodes.forEach(function(d){o[d.name]=[]});function u(d,h){!o[d]||!o[h]||d===h||(o[d].indexOf(h)===-1&&o[d].push(h),o[h].indexOf(d)===-1&&o[h].push(d))}t.links.forEach(function(d){u(d.source,d.target)});const g={};t.nodes.forEach(function(d){const h=r(d.name);g[h]||(g[h]=[]),g[h].push(d.name)}),Object.keys(g).forEach(function(d){const h=g[d];if(!(h.length<2))for(let v=0;v<h.length;v++)for(let E=v+1;E<h.length;E++)u(h[v],h[E])});const f={},y=[];i.forEach(function(d){o[d]!==void 0&&(f[d]=0,y.push(d))});let C=0;for(;C<y.length;){const d=y[C++];(o[d]||[]).forEach(function(h){f[h]===void 0&&(f[h]=f[d]+1,y.push(h))})}Object.keys(f).forEach(function(d){});const I=[{h:260,s:62,l:50},{h:15,s:80,l:55},{h:160,s:60,l:42},{h:220,s:72,l:52},{h:340,s:70,l:52},{h:45,s:85,l:50},{h:190,s:70,l:45},{h:90,s:55,l:45},{h:290,s:60,l:50},{h:30,s:75,l:48},{h:130,s:50,l:42},{h:0,s:70,l:55}];function D(d,h,v){return"hsl("+Math.round(d)+", "+Math.round(h)+"%, "+Math.round(v)+"%)"}const O={},U={},x=t.nodes.filter(function(d){return a[d.name]});x.sort(function(d,h){return h.value-d.value}),x.forEach(function(d,h){const v=I[h%I.length];U[d.name]=v,O[d.name]=D(v.h,v.s,v.l)});const N={};i.forEach(function(d){if(o[d]===void 0)return;const h={};h[d]=0;const v=[d];let E=0;for(;E<v.length;){const R=v[E++];(o[R]||[]).forEach(function(M){h[M]===void 0&&(h[M]=h[R]+1,v.push(M))})}N[d]=h});const J=t.nodes.filter(function(d){return!a[d.name]});let re=1;J.forEach(function(d){let h=1/0;i.forEach(function(v){if(!N[v])return;const E=N[v][d.name];E!==void 0&&E<h&&(h=E)}),h<1/0&&h>re&&(re=h)}),J.forEach(function(d){const h=[];let v=0;if(i.forEach(function(p){if(!N[p]||!U[p])return;let b=N[p][d.name];if(b===void 0)return;b===0&&(b=.5);const L=1/(b*b);h.push({ft:p,w:L}),v+=L}),v===0){O[d.name]="hsl(0, 0%, 82%)";return}let E=0,R=0,M=0,w=0;h.forEach(function(p){const b=p.w/v,L=U[p.ft],_=L.h*Math.PI/180;E+=Math.sin(_)*b,R+=Math.cos(_)*b,M+=L.s*b,w+=L.l*b});let A=Math.atan2(E,R)*180/Math.PI;A<0&&(A+=360);let q=M,F=w,m=1/0;h.forEach(function(p){const b=N[p.ft][d.name];b<m&&(m=b)});let S=(m-1)/Math.max(re-1,1);S=Math.max(0,Math.min(1,S));const P=Math.pow(S,.85),T=32,k=1-P*.35;q=Math.max(T,q*k),F=F+P*(82-F)*.78,O[d.name]=D(A,q,F)});const le=t.tagTranslations||{},ee={};Object.keys(le).forEach(function(d){ee[d]=le[d];const h=d.replace(/-/g," ");h!==d&&(ee[h]=le[d])});function te(d){return(typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en")==="zh-CN"&&ee[d]?ee[d]:d}const ve=e.getBoundingClientRect(),B=ve.width||500,W=ve.height||400,ge=Math.min(B,W)*.28;x.forEach(function(d,h){const v=2*Math.PI*h/Math.max(x.length,1)-Math.PI/2;d.x=B/2+ge*Math.cos(v),d.y=W/2+ge*Math.sin(v)});const ye=Math.min(B,W)*.45;J.forEach(function(d,h){const v=2*Math.PI*h/Math.max(J.length,1);d.x=B/2+ye*Math.cos(v),d.y=W/2+ye*Math.sin(v)});const We=60,z=t.nodes;for(let d=0;d<15;d++)for(let h=0;h<z.length;h++)for(let v=h+1;v<z.length;v++){const E=z[v].x-z[h].x,R=z[v].y-z[h].y,M=Math.sqrt(E*E+R*R),w=We+(z[h].symbolSize+z[v].symbolSize)/2;if(M<w){const A=(w-M)/2,q=M>.1?E/M:Math.random()-.5,F=M>.1?R/M:Math.random()-.5;z[h].x-=q*A,z[h].y-=F*A,z[v].x+=q*A,z[v].y+=F*A}}let K=1,Z=[B/2,W/2];if(x.length>0){let d=1/0,h=-1/0,v=1/0,E=-1/0;x.forEach(function(w){const A=(w.symbolSize||20)/2+50;w.x-A<d&&(d=w.x-A),w.x+A>h&&(h=w.x+A),w.y-A<v&&(v=w.y-A),w.y+A>E&&(E=w.y+A)});const R=h-d,M=E-v;if(R>0&&M>0){const w=B/R,A=W/M;K=Math.min(w,A,1.5)*.8,K<.3&&(K=.3),Z=[(d+h)/2,(v+E)/2]}}t.nodes.forEach(function(d){d.itemStyle={color:O[d.name],borderColor:"#fff",borderWidth:1.5,shadowBlur:5,shadowColor:"rgba(0, 0, 0, 0.06)"},d.label={show:!0,formatter:function(){return te(d.name)},fontSize:Math.max(10,Math.min(15,9+(l[d.name]||0)*.5)),color:"#555",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif'}});const j=e.querySelector(".tag-graph-hint");let be=null,ce=!1;function Ue(){ce||(ce=!0,j.classList.add("visible"),clearTimeout(be),be=setTimeout(function(){j.classList.remove("visible"),ce=!1},3e3))}e.addEventListener("mouseenter",Ue);const G=document.createElement("script");G.src="https://cdn.jsdelivr.net/npm/echarts@5.5.1/dist/echarts.min.js",G.integrity="sha384-Mx5lkUEQPM1pOJCwFtUICyX45KNojXbkWdYhkKUKsbv391mavbfoAmONbzkgYPzR",G.crossOrigin="anonymous",G.onload=function(){Ge()},G.onerror=function(){n.textContent="Failed to load chart library",n.style.color="#c44"},document.head.appendChild(G);function je(d){return d<10?750:d<20?1200:d<40?1650:2100}function Ge(){n.parentNode&&n.parentNode.removeChild(n);const d=window.matchMedia&&window.matchMedia("(pointer: coarse)").matches||"ontouchstart"in window,h=echarts.init(s),v={backgroundColor:"transparent",tooltip:{show:!0,enterable:!0,confine:!0,backgroundColor:"rgba(255, 255, 255, 0.97)",borderColor:"#e8e8e8",borderWidth:1,padding:[10,14],textStyle:{color:"#4b4848",fontFamily:'"noto-sans-sc", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',fontSize:13},extraCssText:"border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); max-height: 260px; overflow-y: auto;",formatter:function(m){function S(T){return String(T??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function P(T,k){const p=S(T),b=S(k),L='style="display:block;color:#666;font-size:11px;line-height:1.6;padding:1px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px;text-decoration:none;"';return b?'<a href="'+b+'" '+L+">• "+p+"</a>":"<div "+L+">• "+p+"</div>"}if(m.dataType==="node"){const T=te(m.name);let k='<div style="font-weight:600;font-size:14px;margin-bottom:5px;color:'+(O[m.name]||"#795da3")+'">'+S(T)+"</div>";k+='<div style="color:#777;font-size:12px;margin-bottom:6px;">📄 '+m.value+" article"+(m.value>1?"s":"")+"</div>";const p=t.postTitles&&t.postTitles[m.name];return p&&p.length>0&&(k+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;">',p.forEach(function(b){typeof b=="string"?k+=P(b,""):k+=P(b.title,b.path)}),k+="</div>"),k}if(m.dataType==="edge"){const T=m.data.source,k=m.data.target;let p='<span style="font-weight:600">'+S(te(T))+'</span> <span style="color:#bbb">↔</span> <span style="font-weight:600">'+S(te(k))+"</span>";p+='<br/><span style="color:#999;font-size:12px">📄 '+m.data.value+" article"+(m.data.value>1?"s":"")+"</span>";const b=[T,k].sort().join("	"),L=t.linkPosts&&t.linkPosts[b];return L&&L.length>0&&(p+='<div style="max-height:160px;overflow-y:auto;border-top:1px solid #eee;padding-top:5px;margin-top:5px;">',L.forEach(function(_){p+=P(_.title,_.path)}),p+="</div>"),p}}},animationDuration:1500,animationEasingUpdate:"quinticInOut",series:[{type:"graph",layout:"force",data:t.nodes,links:t.links,roam:!1,draggable:!d,force:{repulsion:je(t.nodes.length),edgeLength:[150,450],gravity:.12,friction:.6,layoutAnimation:!0},emphasis:{focus:"adjacency",blurScope:"global",itemStyle:{shadowBlur:20,shadowColor:"rgba(121, 93, 163, 0.45)",borderWidth:2,borderColor:"#fff"},lineStyle:{width:3,opacity:.85},label:{show:!0,fontSize:14,fontWeight:"bold",color:"#333"}},label:{position:"right",distance:6},lineStyle:{color:"#d0d0d0",width:1.5,curveness:0,opacity:.35},scaleLimit:{min:.3,max:4},zoom:K,center:Z}]};h.setOption(v);let E=!1,R=!1;if(x.length>0){const m=function(){if(R)return;if(E){R=!0,h.off("finished",m);return}const S=h.getModel(),P=S&&S.getSeriesByIndex&&S.getSeriesByIndex(0),T=P&&P.getGraph&&P.getGraph();let k=1/0,p=-1/0,b=1/0,L=-1/0,_=0;if(x.forEach(function(ie){const de=T&&T.getNodeByName&&T.getNodeByName(ie.name),se=de&&de.getLayout&&de.getLayout();let Y,$;se&&se.length>=2?(Y=se[0],$=se[1]):(Y=ie.x||0,$=ie.y||0);const H=(ie.symbolSize||20)/2+50;Y-H<k&&(k=Y-H),Y+H>p&&(p=Y+H),$-H<b&&(b=$-H),$+H>L&&(L=$+H),_++}),_===0)return;const Q=p-k,ne=L-b;if(Q<=0||ne<=0)return;let V=Math.min(B/Q,W/ne,1.5)*.8;V<.3&&(V=.3),R=!0,M=V,w=[(k+p)/2,(b+L)/2],h.setOption({series:[{zoom:V,center:w.slice()}]}),h.off("finished",m)};h.on("finished",m)}h.on("click",function(m){m.dataType==="node"&&t.tagPaths&&t.tagPaths[m.name]&&(window.location.href=t.tagPaths[m.name])}),h.on("mouseover",function(m){(m.dataType==="node"||m.dataType==="edge")&&(s.style.cursor="pointer")}),h.on("mouseout",function(){s.style.cursor="default"});let M=K||1,w=Z?[Z[0],Z[1]]:[0,0];if(!d){const m=e||s;m.addEventListener("wheel",function(p){p.preventDefault()},{passive:!1}),m.addEventListener("touchmove",function(p){p.touches.length>=2&&p.preventDefault()},{passive:!1});const S=h.getZr();S.on("mousewheel",function(p){p.event.preventDefault(),p.event.stopPropagation(),E=!0;const b=p.wheelDelta>0?1.08:1/1.08;let L=M*b;L<.3&&(L=.3),L>4&&(L=4),M=L,h.setOption({series:[{zoom:M}]})});let P=!1,T=[0,0],k=[0,0];S.on("mousedown",function(p){p.target||(P=!0,E=!0,T=[p.event.clientX,p.event.clientY],k=[w[0],w[1]],s.style.cursor="grabbing")}),S.on("mousemove",function(p){if(P){const b=p.event.clientX-T[0],L=p.event.clientY-T[1],_=s.clientWidth,Q=s.clientHeight,ne=_/M,V=Q/M;w[0]=k[0]-b*(ne/_),w[1]=k[1]-L*(V/Q),h.setOption({series:[{center:[w[0],w[1]]}]})}}),S.on("mouseup",function(){P&&(P=!1,s.style.cursor="default")}),S.on("globalout",function(){P&&(P=!1,s.style.cursor="default")})}if(d){const m=document.createElement("button");m.type="button",m.className="tag-graph-fs-btn",m.setAttribute("aria-label","Fullscreen"),m.innerHTML='<svg class="tag-graph-fs-ic tag-graph-fs-ic--open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 3 21 3 21 9"></polyline><polyline points="9 21 3 21 3 15"></polyline><line x1="21" y1="3" x2="14" y2="10"></line><line x1="3" y1="21" x2="10" y2="14"></line></svg><svg class="tag-graph-fs-ic tag-graph-fs-ic--close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',e.appendChild(m);let S=!1;const P=function(){if(!j)return;const p=typeof localStorage<"u"&&localStorage.getItem("siteLanguage")||"en";j.textContent=p==="zh-CN"?"双指缩放 · 拖动平移 · 点按进入标签":"Pinch to zoom · Drag to pan · Tap a tag",j.classList.add("visible"),setTimeout(function(){j.classList.remove("visible")},2600)},T=function(){S=!0,e.classList.add("tag-graph-fullscreen"),m.classList.add("is-fullscreen"),m.setAttribute("aria-label","Exit fullscreen"),document.body.style.overflow="hidden",document.body.classList.add("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!0}]}),P()})},k=function(){S=!1,e.classList.remove("tag-graph-fullscreen"),m.classList.remove("is-fullscreen"),m.setAttribute("aria-label","Fullscreen"),document.body.style.overflow="",document.body.classList.remove("tag-graph-fs-active"),requestAnimationFrame(function(){h.resize(),h.setOption({series:[{roam:!1,zoom:M,center:w.slice()}]})})};m.addEventListener("click",function(p){p.preventDefault(),p.stopPropagation(),S?k():T()}),document.addEventListener("keydown",function(p){p.key==="Escape"&&S&&k()})}let A;window.addEventListener("resize",function(){clearTimeout(A),A=setTimeout(function(){h.resize()},150)});function q(){h.setOption({series:[{data:t.nodes}]})}window.addEventListener("storage",function(m){m.key==="siteLanguage"&&q()});const F=localStorage.setItem;localStorage.setItem=function(m,S){F.call(localStorage,m,S),m==="siteLanguage"&&setTimeout(q,50)}}}function Bt(s){if(s.querySelector("audio"))return!1;const e=s.querySelector("summary");return!(!e||/跟读|本节语音/.test(e.textContent||"")||s.parentElement&&s.parentElement.closest("details.callout"))}function Wt(s,e={}){const t=e.includeCallouts?"h1, h2, h3, h4, h5, h6, details.callout--foldable":"h1, h2, h3, h4, h5, h6",n=Array.from(s.querySelectorAll(t)),i=[];let a=1;const r=[];return n.forEach(l=>{const c=/^H[1-6]$/.test(l.tagName);let o,u,g=!1;if(c)o=parseInt(l.tagName[1],10),a=o,u=l.textContent;else{if(!Bt(l))return;o=a+1,g=!0;const I=l.querySelector("summary");if(u=Pe(I.textContent||"",l.getAttribute("data-callout")||"",{short:!0}),!u)return}let f=1;for(;i.length&&i[i.length-1].level>=o;){const I=i.pop();I.level===o&&(f=I.n+1)}i.push({level:o,n:f});const y=r.length;l.id||(l.id=g?`toc-item-${y}`:`heading-${y}`);const C=i.map(I=>I.n).join(".");l.dataset.tocNumber=C,r.push({element:l,level:o,index:y,id:l.id,text:u,number:C,virtual:g})}),r}function Ut(s){const e=document.createElement("aside");e.className="toc-drawer",e.innerHTML='<div class="toc-header"><span class="toc-title"></span><div class="toc-header__actions"><button type="button" class="toc-foldall"></button><button type="button" class="toc-close-btn"><span class="toc-close-btn__x" aria-hidden="true"></span></button></div></div><div class="toc-content"><div class="toc-list"></div></div>',document.body.appendChild(e);const t=e.querySelector(".toc-list"),n=s.map(i=>{const a=document.createElement("div");if(a.className=i.virtual?"toc-item toc-item--virtual":"toc-item",a.setAttribute("data-level",String(i.level)),a.setAttribute("data-index",String(i.index)),!i.virtual){const l=document.createElement("div");l.className="toc-collapse-btn",a.appendChild(l)}const r=document.createElement("span");return r.className="toc-item-text",r.style.cursor="pointer",r.innerHTML=`<span class="toc-number">${i.number}.</span> `,r.appendChild(document.createTextNode(i.text)),r.setAttribute("title",i.text),i.element.classList.contains("collapsed")&&a.classList.add("collapsed"),a.appendChild(r),t.appendChild(a),a});return{container:e,items:n}}const Oe="toc-panel-state";function He(){try{const s=localStorage.getItem(Oe);if(!s)return null;const e=JSON.parse(s);return!e||typeof e!="object"?null:e}catch{return null}}function jt(s){try{const t={...He()||{},...s};localStorage.setItem(Oe,JSON.stringify(t))}catch{}}function Gt(){return(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?"zh":"en"}const Vt={zh:{title:"目录",open:"目录",hide:"收起目录",show:"打开目录"},en:{title:"Contents",open:"TOC",hide:"Hide contents",show:"Show contents"}};function Yt(s){const e=s.querySelector(".toc-content"),t=s.querySelector(".toc-item.toc-reading");!e||!t||(e.scrollTop=t.offsetTop-e.clientHeight/2+t.offsetHeight/2)}function $t(s){const e=Vt[Gt()],t=s.querySelector(".toc-title");t&&(t.textContent=e.title),s.setAttribute("aria-label",e.title);const n=s.querySelector(".toc-close-btn");n&&(n.setAttribute("aria-label",e.hide),n.setAttribute("title",e.hide));const i=document.createElement("button");i.type="button",i.className="toc-tab",i.setAttribute("aria-label",e.show),i.innerHTML='<span class="toc-tab__icon" aria-hidden="true"></span><span class="toc-tab__text">'+e.open+"</span>",document.body.appendChild(i);const a=document.createElement("div");a.className="toc-scrim",document.body.appendChild(a);function r(o,u){s.classList.toggle("is-open",o),i.classList.toggle("is-hidden",o),a.classList.toggle("is-visible",o),document.body.classList.toggle("toc-drawer-open",o),o&&Yt(s),u&&jt({hidden:!o})}i.addEventListener("click",()=>r(!0,!0)),a.addEventListener("click",()=>r(!1,!0)),n&&n.addEventListener("click",o=>{o.stopPropagation(),r(!1,!0)}),document.addEventListener("keydown",o=>{o.key==="Escape"&&s.classList.contains("is-open")&&r(!1,!1)});const l=He(),c=window.matchMedia("(min-width: 1100px)").matches;return r(c&&!!l&&l.hidden===!1,!1),{setOpen:r}}function me(s,e,t){for(let n=e+1;n<s.length&&!(parseInt(s[n].getAttribute("data-level")||"1",10)<=t);n+=1)s[n].classList.add("toc-hidden")}function Be(s,e,t){for(let n=e+1;n<s.length;n+=1){const i=parseInt(s[n].getAttribute("data-level")||"1",10);if(i<=t)break;if(i===t+1)s[n].classList.remove("toc-hidden");else{let a=!0;for(let r=n-1;r>e;r-=1){const l=parseInt(s[r].getAttribute("data-level")||"1",10);if(l<i&&s[r].classList.contains("collapsed")){a=!1;break}if(l<=t)break}a&&s[n].classList.remove("toc-hidden")}}}function Xt(s){const e=parseInt(s.tagName.charAt(1),10);let t=s.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t=t.nextElementSibling;continue}t.style.display="none",t=t.nextElementSibling}}function Kt(s){const e=parseInt(s.tagName.charAt(1),10);let t=s.nextElementSibling;for(;t&&!(t.tagName&&/^H[1-6]$/.test(t.tagName)&&parseInt(t.tagName.charAt(1),10)<=e);){if(t.classList&&t.classList.contains("tags")){t.style.display="",t=t.nextElementSibling;continue}t.style.display="",t=t.nextElementSibling}}function pe(s,e,t){const n=e[t],i=s[t]&&s[t].element;if(!n||!i)return;const a=parseInt(n.getAttribute("data-level")||"1",10);!n.classList.contains("collapsed")?(n.classList.add("collapsed"),me(e,t,a),i.classList.contains("collapsed")||(i.classList.add("collapsed"),Xt(i))):(n.classList.remove("collapsed"),Be(e,t,a),i.classList.contains("collapsed")&&(i.classList.remove("collapsed"),Kt(i)))}function Zt(s,e,t){e.forEach((n,i)=>{if(n.classList.contains("toc-item--virtual")||!n.querySelector(".toc-collapse-btn"))return;const a=n.classList.contains("collapsed");t?!a&&!n.classList.contains("toc-hidden")&&pe(s,e,i):a&&pe(s,e,i)})}function Qt(s,e,t){const n=e[t],i=s[t]&&s[t].element;if(!n||!i)return;const a=parseInt(n.getAttribute("data-level")||"1",10),r=i.classList.contains("collapsed");r&&!n.classList.contains("collapsed")?(n.classList.add("collapsed"),me(e,t,a)):!r&&n.classList.contains("collapsed")&&(n.classList.remove("collapsed"),Be(e,t,a))}function Jt(s,e){e.forEach((i,a)=>{if(i.classList.contains("collapsed")){const r=parseInt(i.getAttribute("data-level")||"1",10);me(e,a,r)}}),e.forEach((i,a)=>{const r=i.querySelector(".toc-collapse-btn");r&&r.addEventListener("click",l=>{l.stopPropagation(),pe(s,e,a)})});const t=new Map;s.forEach((i,a)=>t.set(i.element,a));const n=new MutationObserver(i=>{i.forEach(a=>{if(a.type!=="attributes"||a.attributeName!=="class")return;const r=t.get(a.target);r!==void 0&&Qt(s,e,r)})});return s.forEach(i=>{n.observe(i.element,{attributes:!0,attributeFilter:["class"]})}),{observer:n}}const Te={1:{passed:"rgba(128,128,128,0.1)",reading:"rgba(66,153,225,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(66,153,225,0.25)"},2:{passed:"rgba(128,128,128,0.1)",reading:"rgba(49,130,206,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(49,130,206,0.25)"},3:{passed:"rgba(128,128,128,0.1)",reading:"rgba(44,82,130,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(44,82,130,0.25)"},4:{passed:"rgba(128,128,128,0.1)",reading:"rgba(42,67,101,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(42,67,101,0.25)"},5:{passed:"rgba(128,128,128,0.1)",reading:"rgba(26,54,93,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(26,54,93,0.25)"},6:{passed:"rgba(128,128,128,0.1)",reading:"rgba(21,62,117,0.15)",coming:"rgba(200,200,200,0.05)",active:"rgba(21,62,117,0.25)"}};function en(s,e){if(!s.length)return{destroy(){}};const t=new Array(s.length).fill("coming"),n=new Set;let i=-1;const a=new Map;s.forEach((f,y)=>a.set(f.element,y));function r(){e.forEach((f,y)=>{const C=parseInt(f.getAttribute("data-level")||"1",10),I=Te[C]||Te[1],D=t[y];f.classList.remove("toc-passed","toc-reading","toc-coming"),f.style.boxShadow="",f.style.transform="",f.style.fontWeight="",y===i?(f.classList.add("toc-reading"),f.style.backgroundColor=I.active,f.style.opacity="1",f.style.fontWeight="600",f.style.boxShadow="inset 0 0 0 2px rgba(66,153,225,0.3)",f.style.transform="scale(1.02)",f.style.transition="all 0.2s ease"):D==="reading"?(f.classList.add("toc-reading"),f.style.backgroundColor=I.reading,f.style.opacity="1",f.style.fontWeight="600"):D==="passed"?(f.classList.add("toc-passed"),f.style.backgroundColor=I.passed,f.style.opacity="0.7"):(f.classList.add("toc-coming"),f.style.backgroundColor=I.coming,f.style.opacity="0.5")})}function l(){const f=window.innerHeight/2;let y=-1;n.forEach(C=>{const I=s[C].element.getBoundingClientRect();I.top<=f&&I.bottom>=f&&(y=C)}),y!==i&&(i=y,r())}let c=null;function o(){c||(c=requestAnimationFrame(()=>{c=null,l()}))}const u=new IntersectionObserver(f=>{f.forEach(y=>{const C=a.get(y.target);C!==void 0&&(y.isIntersecting?(n.add(C),t[C]="reading"):(n.delete(C),t[C]=y.boundingClientRect.bottom<0?"passed":"coming"))}),l(),r()});s.forEach(f=>u.observe(f.element)),window.addEventListener("scroll",o,{passive:!0}),window.addEventListener("resize",o,{passive:!0}),r();function g(){u.disconnect(),window.removeEventListener("scroll",o),window.removeEventListener("resize",o),c&&cancelAnimationFrame(c)}return{destroy:g,refresh:()=>{l(),r()}}}const tn='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M7.41 18.59 8.83 20 12 16.83 15.17 20l1.41-1.41L12 14zM16.59 5.41 15.17 4 12 7.17 8.83 4 7.41 5.41 12 10z"/></svg>',nn='<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 5.83 15.17 9l1.41-1.41L12 3 7.42 7.59 8.83 9zm0 12.34L8.83 15l-1.41 1.4L12 21l4.58-4.6L15.17 15z"/></svg>';function sn(s){return!s.classList.contains("toc-item--virtual")&&!!s.querySelector(".toc-collapse-btn")}function an(){const s=document.querySelector(".content");if(!s||s.classList.contains("archives"))return null;const e=document.body;if(e.classList.contains("path-about-index-html")||e.classList.contains("layout-about"))return null;const t=document.querySelector("section.main[data-toc]");return t&&t.getAttribute("data-toc")==="false"?null:s}function on(){const s=an();if(!s)return;const e=Wt(s,{includeCallouts:document.body.classList.contains("type-daily-feed")});if(!e.length)return;const{container:t,items:n}=Ut(e);Jt(e,n);const i=en(e,n);$t(t);const a=t.querySelector(".toc-foldall"),r=n.filter(sn);if(a&&r.length){const c=(document.documentElement.getAttribute("lang")||"").toLowerCase().indexOf("zh")===0?{collapse:"全部折叠",expand:"全部展开"}:{collapse:"Collapse all",expand:"Expand all"},o=()=>{const u=r.some(g=>!g.classList.contains("collapsed"));a.innerHTML=u?tn:nn,a.setAttribute("aria-label",u?c.collapse:c.expand),a.setAttribute("title",u?c.collapse:c.expand)};a.addEventListener("click",()=>{const u=r.some(g=>!g.classList.contains("collapsed"));Zt(e,n,u),o()}),o()}else a&&a.remove();n.forEach((l,c)=>{const o=l.querySelector(".toc-item-text");o&&o.addEventListener("click",()=>{const u=e[c];!u||!u.element||(u.virtual&&u.element.tagName==="DETAILS"&&!u.element.open&&(u.element.open=!0),u.element.scrollIntoView({behavior:"smooth",block:u.virtual?"start":"center"}),setTimeout(()=>i.refresh(),300))})})}function Ie(){new Ve,new ot,new xt,document.getElementById("map")&&new Et,new St,setTimeout(()=>{new Lt},500),new Ct,new kt,new It,Ot(),on(),new wt,document.getElementById("tag-graph")&&Ht()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ie):Ie();
